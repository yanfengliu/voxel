...
- Promote scene-enabling mechanisms into Voxel only as bounded, game-neutral capabilities reusable by downstream games; keep each scene's choreography, scripted trajectories, physics or simulation, causal event rules, and game semantics in Studio or consumer fixtures or game-owned adapters, with the renderer observing versioned plain-data state or deterministic traces.
