...
The repository contains the private Version 1.0 release: strict-TypeScript package exports, bounded snapshots and atomic deltas, visible-face oracle and greedy production meshers, a packaged stale-safe worker pipeline, a bounded Three-free dense-chunk occupancy ray query, and a Three.js WebGL runtime with accepted/presented separation, revision-atomic frame commits, committed voxel/instance picking, and revision-aware capture. AoE2 proves the standalone sole-renderer host, while City proves an embedded borrowed-renderer instance lane. Committed `pickPresented` queries are available only for worlds whose descriptor carries `chunkProfile` and whose runtime enables `voxelWorkers`; an otherwise-running world outside that path reports `no-presented-frame`, while lost, restoring, failed, and disposed runtimes report their own typed unavailable outcomes. Imported asset APIs, public extension or logical-pick-proxy APIs, spatial sharding, and broader consumer adoption including Townscaper remain future work; registry publication remains a separate decision. Do not claim that a package, command, backend, benchmark, shader, mesher, adapter, or test exists until live files and executable behavior prove it.
