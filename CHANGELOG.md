...
- Added an internal revision-atomic target coordinator that joins whole-target worker admission, deterministic multi-group completion, off-scene Three staging, supersession and epoch fencing, zero-job targets, terminal cleanup, and retryable idempotent worker/stager disposal. Runtime frame integration remains open.
