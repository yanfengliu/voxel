...
- Added an internal revision-atomic target coordinator that joins whole-target worker admission, deterministic multi-group completion, off-scene Three staging, supersession and epoch fencing, zero-job targets, generation-aware worker-crash retry/failure routing, terminal cleanup, and retryable idempotent worker/stager disposal. The legacy runtime now centralizes its root, presenters, animation, metrics, reset, and disposal behind one presentation-surface owner; atomic frame integration remains open. The callable Three API is unchanged, while its declaration hash records TypeScript's private-field layout change.
