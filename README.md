...
The repository also hosts the model studio (`npm run studio`), a browser workbench for cataloged parts, recipes, models, and scenes, with editing, captured-view review, deterministic replay inspection, and bounded Studio-only rendering experiments. Content and simulation semantics remain owned by the catalog producer; Voxel presents only versioned data. See [the model studio guide](docs/guides/model-studio.md#scenes).
