...
The repository also hosts the model studio (`npm run studio`) — a browser workbench for the voxel models the games ship: frame-by-frame playback, pinned notes, agent-drivable controls, models saved as the recipes that made them, and scenes that stand those models together in one world with editable local point lights ([design](docs/superpowers/specs/2026-07-17-model-recipes-and-shared-parts-design.md)). Its internal clustered-light proof accepts up to 4,096 authored point lights, evaluates at most 32 local influences per fragment, draws every light marker in one instanced call, and includes a built-in scene with 1,000 independently orbiting lights; it has no shadows and is not a public 1.0 dynamic-light API. [The model studio guide](docs/guides/model-studio.md#scenes) records the proof's exact limits and current transaction boundary.

A sealed [named-hardware benchmark](benchmarks/results/2026-07-26T05-12-53-840Z-clustered-lights-1000.json) of commit `1f4377d` measured the 1,000-moving-light scene at 1,201.2 GPU-synchronized uncapped Studio draws/s on an RTX 4090 at 1280×720, with 0.80/0.90/1.10 ms p50/p95/p99 and an 826.4 draws/s slowest-1%-mean rate across 3,000 fixed-sequence frames. That is draw throughput for a small 35-placement receiver scene—not display-refresh FPS, gameplay FPS, or evidence for a full game-sized world.
