...
The repository also hosts the model studio (`npm run studio`) — a browser workbench for the voxel models the games ship: frame-by-frame playback, pinned notes, agent-drivable controls, models saved as the recipes that made them, and scenes that stand those models together in one world with editable local point lights ([design](docs/superpowers/specs/2026-07-17-model-recipes-and-shared-parts-design.md)). Its internal clustered-light proof accepts up to 4,096 authored point lights, evaluates at most 32 local influences per fragment, draws every light marker in one instanced call, and includes a built-in proof where 1,000 full-spectrum lights move on three axes around 1,000 neutral receivers; the global lighting control reports its persisted on/off state explicitly, and an off source remains only as a dim edit handle that does not orbit during playback. The proof has no shadows and is not a public 1.0 dynamic-light API. [The model studio guide](docs/guides/model-studio.md#scenes) records the proof's exact limits and current transaction boundary.
