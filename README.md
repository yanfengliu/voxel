...
**Version 1.1.0** — stable. Foundations, atomic deltas, the production voxel pipeline, committed presented-state picking for profiled `voxelWorkers` worlds, host/camera/context lifecycle, and both consumer proofs are delivered: AoE2 runs Voxel standalone as its sole world renderer, and City draws its building wall lane through an embedded, borrowed-renderer runtime. The frozen 1.0 candidate's hardening evidence records 760 unit tests across 92 files, 11 real-browser tests including visual baselines, a pinned hostile-input fuzz corpus, endurance runs that hold resource counts flat over 1,000 edits, and 30 real device losses proving reconstruction — a context loss resets Three's memory counters, so the loss lane cannot see a leak and the flat-resource claim rests on the repeated-edit test alone, a supply-chain gate, green Windows/Linux CI, and eleven named reference fixtures measured on named hardware ([benchmarks/results/](benchmarks/results/)) — a boundary edit through the worker path presents in ~38 ms p50, a 50k-instance sparse patch in ~1.3 ms, a pick query in ~0.1 ms on an RTX 4090. Post-release model-studio work has grown coverage well past that frozen record — 227 unit-test files and 22 real-browser spec files as of 2026-08-13 — and the complete gate stays green; `npm run verify` is the current count, not this sentence.

Public declarations, schema literals, delta operation discriminants, and the support matrix are frozen; changing any of them means a major release. [The support policy](docs/policies/support.md) defines the supported platforms and the evidence each claim rests on, and [the changelog](CHANGELOG.md) records what each release contains. Distribution is a private tag and packed artifact; registry publication is a separate decision.
...
See [the consumer integration guide](docs/guides/consumer-integration.md) for the lifecycle and game-owned adapter boundary, and [the model studio guide](docs/guides/model-studio.md) for standing up a game's own studio. The private Three-free `@voxel/model-studio-ui` file package owns one workbench, five-tab vocabulary, accessibility contract, and visual baseline for every game; renderer and content adapters keep each game's catalog, assets, and review semantics. [The current engine design](docs/design/spec.md) and [0.1 implementation ledger](docs/plans/implementation.md) record the delivered vertical slice. The delivered 1.0 record is [the roadmap](docs/plans/v1-roadmap.md), [target architecture](docs/design/v1-architecture.md), and [implementation plan](docs/plans/v1-implementation.md); [the support policy](docs/policies/support.md) defines supported platforms and the evidence behind each claim. [The ecosystem review](docs/research/ecosystem.md) records which mature libraries are adopted, evaluated, or rejected.

Use the [Create Diverse Voxel Assets skill](docs/skills/create-diverse-voxel-assets/SKILL.md) for the end-to-end engine and downstream-game workflow: creative briefs, structural exploration, reproducible recipes, evidence-driven promotion, coherent asset systems, and consumer-boundary verification.

The repository also hosts the model studio (`npm run studio`), a browser workbench for cataloged parts, recipes, and models, with editing, captured-view review, deterministic replay inspection, and bounded Studio-only rendering experiments. Content and simulation semantics remain owned by the catalog producer; Voxel presents only versioned data. See [the model studio guide](docs/guides/model-studio.md).
