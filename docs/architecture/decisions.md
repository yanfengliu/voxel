...
- Current slice: Model Studio rejects any solid voxel shared by two distinct recipe occurrences. Direct construction steps inside one occurrence may still layer intentionally. This is an exact authoring-time grid invariant, not a runtime-physics claim.
