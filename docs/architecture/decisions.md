...
- Status: accepted as a design direction on 2026-07-20; runtime physics is not implemented in Voxel, and one development-only consumer fixture now exercises part of the boundary.
...
- Current slice: Model Studio rejects any solid voxel shared by two distinct recipe occurrences, and a versioned `PhysicalAssetV1` sidecar beside a saved recipe now authors bodies, colliders, fixed/revolute/prismatic joints, and attachment ports, compiled per occurrence — mirrors included — into distinct stably named bodies. Direct construction steps within one occurrence may still layer intentionally. A fixture-local adapter maps nine causal Machine Works sidecars into pinned Rapier compound shapes, materials, CCD, sensors, and ports, while a tenth exact exposed-cog sidecar remains cataloged and reusable without entering Rapier. Fifty-eight 26-voxel-deep kinematic slats circulate around two internal drive drums with their undersides following the nominal 2.75-unit pitch datum and bounded articulation gaps. An exact sidecar OBB/SAT regression samples every slat against both pitch drums across 32 phases, proving that slat-edge and drum-end-cheek compounds share boundary only with zero positive-volume overlap, that the central barrel keeps at least about 0.275 world units of radial clearance, and that a shifted negative control is detected. Four exposed axle-cog replay tracks derive from the solved drive-drum poses outside the solver, so all 64 presented moving elements share one hashed closed-loop drive phase while the cogs prove only visible axle-phase agreement and allocate no physics handles. Contact and friction transport an axis-constrained dynamic carrier aligned with visible guards through validated assembly stations without prescribing its X pose, claiming guard contact, or attributing transport to the exposed cogs. Paired same-geometry 240-tick ablations recreate the complete causal world of foundation, slats, drive drums, carrier, and jointed load and record maximum displacement over the same interval: zero drive must remain within 0.05 world units, and driven zero friction must remain within 20 percent of the driven trace's maximum. Only a carrier whose full X, Y, Z, orientation, and speed state passes the hashed output tolerances may enter the explicit position-based tip servo, which starts from the exact accepted dynamic pose without a snap. The consumer generates a deterministic 71-track, 1,800-frame, 30-second assembly, release, contact, and collection trace while private read-only Studio V4 presents 73 instances and replays those observations through ordinary sparse render deltas, holding the final frame until the discrete 30-second reset. This is a bounded proof of synchronized frictional transport and the recipe-sidecar-trace boundary, not Voxel runtime physics, exposed-cog contact, cog torque, belt tension, tooth engagement, arbitrary-load no-slip behavior, or a reusable production simulation package.
...
- Adoption rule: the single-purpose Rapier fixture may remain a development-only dependency because fixed-step replay, compound bodies, fixed joints, contact evidence, continuous collision detection, supply-chain, and render-delta projection are now proven there. Do not extract a shared simulation package until reusable adapter design, overlap and swept-query behavior, the full generic joint set, browser lifecycle, teardown, rollback/persistence, picking parity, performance, and a second consumer are also proven.
