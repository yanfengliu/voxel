...
- Current slice: Model Studio rejects any solid voxel shared by two distinct recipe occurrences, and a versioned `PhysicalAssetV1` sidecar beside a saved recipe now authors bodies, colliders, fixed/revolute/prismatic joints, and attachment ports, compiled per occurrence — mirrors included — into distinct stably named bodies. Direct construction steps inside one occurrence may still layer intentionally. A fixture-local adapter maps the exact Machine Works sidecars into pinned Rapier compound shapes, materials, CCD, sensors, and ports; the headless consumer validates mating frames and dwell, retains collision on the visibly servo-tipped carriage, and generates a deterministic compound-body assembly, release, contact, and collection trace while private read-only Studio V4 only replays those observations as render deltas. This is one bounded consumer proof, not Voxel runtime physics or a reusable production simulation package.
