...
The built-in Riverfall canyon is no longer an authored-path visual-flow reference; it is the narrow fixture-local fluid reference. Seven ordinary opaque-voxel recipes and one reused seed-varying tree recipe compose static presentation geometry, while an exact twelve-reach sidecar carries a deterministic 2D PBF thin-sheet/surface model from river through lip, fall, three pond reaches, outflow, and visible submergence before an occluded external pump returns particles through the sink, under-foundation reach, source rise, and visible emergence. Each production trace advances 288 fixed-mass particles into 600 genuine 10 ms frames with two fixed 5 ms substeps per frame, using projected gravity, a fixed-order Jacobi density constraint, XSPH, explicit lip attachment, and dissipative fall-to-pond impact; 96 stable witnesses become the generated input- and final-hash-bearing replay that private Studio V4 presents through ordinary sparse instance deltas. The last recorded frame is genuine and held for its final 10 ms interval before one discrete reset, not rewritten to fabricate a periodic endpoint. Fixture tests independently ablate gravity, hidden-pump forcing, the density solve, and XSPH while holding the remaining world fixed. Reuse the recipe-sidecar-solver-trace boundary, not a water-specific renderer feature: this proof covers bounded deterministic 2D surface/thin-sheet dynamics, not volumetric or free-surface Navier-Stokes, deforming water, transparency or refraction, erosion or hydrology, or gameplay water.
