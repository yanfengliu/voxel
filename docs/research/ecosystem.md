...
**The real portability risk is our own arithmetic, not the version.** Rapier requires that every value used to initialize a simulation come from cross-platform deterministic operations, and ECMA-262 leaves `sin`, `cos`, `tan`, their inverses and hyperbolics, `exp`, `log`, `pow`, `cbrt` and `hypot` implementation-approximated — two engines may differ in the last bit. All four fixtures were audited empirically by perturbing every approximated `Math` function one ULP and regenerating each committed trace. The audit tool caught two of its own measurement errors before it caught anything real, which is why the perturbation method is the one to trust over reading call sites: import-time constants hide from call-time probes.

Status by fixture:

- **Chain: fixed and proven stable.** The recorded run starts from frozen literals, and the ring's membership test now compares squared integers instead of `Math.hypot`, whose last bit decided whether boundary cells were in the ring at all.
- **Machine Works: fixed and proven stable.** The belt's prescribed slat poses were computed per tick with `Math.sin`/`Math.cos` and land directly in the trace; they now use fixed-polynomial `deterministicSinV1`/`deterministicCosV1` (`tools/studio/deterministic-trig.ts`), whose exact arithmetic gives identical bits on every engine. Recorded merge angles from `Math.acos`, which has no exact form, are quantized to nine decimals — far finer than the 0.03 rad gate they are judged by, far coarser than one ULP. `Math.hypot` sites became sum-of-squares-then-`sqrt`, which IEEE-754 does require to be correctly rounded. The trace was regenerated once for the arithmetic change and now survives whole-process perturbation of all twenty-two approximated functions.
- **Windmill: exposed but fail-closed.** Its geometry fingerprint depends on `Math.hypot` alone (isolated by single-function perturbation). The frozen-selection guard refuses to generate when the fingerprint drifts, so a different engine gets a loud red test, never a silently different trace. Making it portable is not an edit but a regeneration: the selection chain — 144-candidate manifest, search evidence and proof hashes — was recorded under `hypot`, so the frozen search must be deliberately re-run under exact arithmetic and re-frozen. Named follow-up; do not change the geometry arithmetic without that re-run, because the swap alone turns every windmill suite red at import.
- **Riverfall: exposed and silent.** Its pure-JS PBF solver runs on approximated `Math` every substep, and one ULP anywhere amplifies chaotically — the perturbed run diverges macroscopically (impact counts 130 → 135). Fixing it means moving the solver's per-step arithmetic onto the deterministic helpers and accepting one macroscopic trace change, with the visual baselines re-inspected and regenerated. Named follow-up; until then the Riverfall trace regenerates only on engines whose libm matches the recording machine's.
