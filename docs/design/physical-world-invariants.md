...
A joint in this solver is frictionless, which means a jointed machine has no way to stop: the trebuchet's arm and counterweight swept 896 and 1,398 degrees over 60 simulated seconds with neither ever sleeping, because nothing had been asked to slow them. Axle friction and air drag are supplied as one declared coefficient per body, the same shape as the ball's rolling resistance and for the same reason — a rigid-body solver produces neither on its own. Both are stated with their measured cost, and both carry a counter-run that fails when the constant is stripped.
