...
Only recipe occupancy currently applies as a global invariant. Built-in static scenes are checked by tests through `sceneOverlapsV1`, not by the editor or scene builder, and replay-driven placements are excluded; Machine Works and Windmill implement narrow fixture-specific portions of the rigid rows, while Riverfall implements a narrow fixture-specific portion of the fluid rows. The scene-store migration must move static overlap checking into the save and promotion transaction so catalog integrity does not depend on remembering a test.
...
Accepted design direction on 2026-07-28. The decomposition is implemented in `tools/studio/voxel-colliders.ts` and now feeds solvers directly in three places — the chain fixture, the Studio's live Interact lane, and the physics playground's twin lanes — while no sidecar yet declares derived colliders, so the validator's hand-authored shape list still stands for `PhysicalAssetV1`. Every model in this repository is a voxel grid, and no cell is more privileged than another, so collider authoring should not require a human to pick which parts of a model get an accurate shape.
...
The open questions narrow but stand. The playground's stress preset gives the collider budget its first datum — five hundred single-collider bodies settle at about 0.43 ms per fixed step on the named benchmark host, measured when the headless lane still stepped four times finer than the shared rate (i9-13900KF, Node 22, single-threaded wasm; the machine recorded in `benchmarks/results/`), far under real-time, with the many-collider-per-body island cost still unmeasured — and determinism across mirrored and nested occurrences remains untested because the decomposition runs on a built model rather than on a recipe occurrence.
...
Beside the recorded proofs sits one diagnostic consumer: the physics playground ([guide](../guides/physics-playground.md)), six stations plus three load presets defined once and built identically by a headless vitest fixture and the Studio's live Interact lane. It exercises gravity, resting contact, Coulomb friction thresholds by material, momentum transfer across mass ratios, continuous-collision detection against a deliberately tunnelable wall, contact-only structural support and collapse, faceted rolling, and body-count scaling, and it reports compact toleranced verdicts rather than hashes — the live lane stays unrecorded, and only the deterministic fixture side is evidence. Its measured findings (facet-rest thresholds, the unstable faceted inertia race, restitution creep on slopes, spawn-overlap ejection) are recorded in the guide rather than restated here.

A joint in this solver is frictionless, which means a jointed machine has no way to stop: the trebuchet's arm and counterweight swept 896 and 1,398 degrees over 60 simulated seconds with neither ever sleeping, because nothing had been asked to slow them. Axle friction and air drag are supplied as laws rather than as per-body constants — `voxel/physics` states them, and they reach every body at the site where rigid bodies are created, for the same reason rolling resistance does: a rigid-body solver produces none of them on its own. Content may tune a law per material and may not exempt itself from one; no body in this repository declares any of the three, and a test walks every station to keep it that way. Each is stated with its measured cost and carries a counter-run that fails when the law is overridden back to zero.

The sixth station is the first jointed machine and the first evidence about constraints in this repository: a trebuchet whose axle, counterweight hinge, and sling pivot are revolute joints and whose trigger is a rope released whole by a scripted detachment. It answers one open question and opens another. Answered, partly: a constraint's ends can anchor on drawn geometry and be held there by test — each of the three hinges anchors on a drawn rod inside a drawn ring, and `playground-trebuchet.test.ts` fails if a ring's hole and its joint anchor drift apart. The limits are worth stating exactly, because adversarial review found the first version of this sentence overclaiming. The rope constraint has no drawn rope, only drawn ends. And anchoring is not bearing: the frame is a fixed body that nothing touches, so deleting its colliders changes nothing about the throw. A revolute joint needs no geometry to hold an axle, which means drawn structure around a joint is legibility, and the ledger has to record it as such rather than as a load path. Opened: joint limits, motors, and joint-reaction readback are all still absent, so the station can prove a hinge bears load and a rope holds torque, but nothing here yet measures what a constraint costs the solver, what a joint chain does to determinism at scale, or how a driven joint behaves under load. Rapier's documented joint-limit and motor surface is the obvious next thing to bind, and the suspension cart is the machine that would demand it.

Physical law is enforced here by executable check, not by assertion, and every law that is claimed has a counter-run that fails it. Conservation of energy is a `motion-rule` node in the trebuchet's purpose graph: a passive machine's mechanical energy must fall monotonically, accounted as translational plus rotational kinetic energy — rotation taken in each body's principal frame — plus gravitational potential. Comparing each frame to the one before it rather than to the opening total is what gives it teeth: a fixed opening ceiling grows slack as the machine dissipates, and review measured roughly 7,000 J of injection hiding under one by mid-run. Sensitivity is stated with the check — 1% allowance against a 0.43% measured noise floor, detecting impulses down to roughly a 8.5 m/s kick — because a law whose resolution is unstated is a law nobody can rely on. Newton's third law is bound at the collision station by a mid-air pair whose total momentum drifts 0.002 across the collision once gravity's exact impulse is removed, against a control accounting one body of the same pair that drifts 427.4; it is deliberately not claimed on the ground, where the floor takes momentum through friction at 4.6% across a four-tick window and 7.9% across the eight-tick window the sampling stride resolves. Newton's second law is the falling station's equal-acceleration check, comparing a stone and a wood cube of equal size within 4%. The first law carries no check of its own and is not claimed to.

The lesson underneath is that a missing force is not the same as a wrong one. Coulomb friction cannot slow a rolling sphere — there is no sliding at the contact to do work — so a rigid ball rolls at constant speed forever, and that is the solver being right rather than broken. What is missing is rolling resistance, which comes from deformation that rigid bodies do not have. Rolling resistance is a contact force, so it is gated on an actual contact — that is the correct model, and it is the reason rather than a measured penalty. Being exact about the size of the correction: an earlier version of this paragraph claimed always-on damping cost a projectile nearly a metre of arrival height, and adversarial review could not reproduce it and showed the mechanism does not permit it, so that claim is withdrawn. Measured, gating changes the trebuchet's wall arrival by about 0.15 m of height and its impact tick by single digits. Any modelled force added here carries its constant, its measured effect, and the boundary of what it does not claim.
...
`@dimforge/rapier3d-compat` `0.19.3` is pinned as a development-only dependency for the headless `fixtures/machine-works-consumer` proof. Studio catalogs twelve exact sidecars; nine causal sidecars enter Rapier as rigid bodies, compound colliders, fixed joints, materials, CCD, sensors, and named ports, while the press bridge, output trunnion dock, and exposed phase flags remain hashed and validated outside the solver. The bridge sidecar matches its painted solids, four named feet terminate on distinct occupied foundation-pad top faces, and rear head pads remain tangent to straight faces as visual alignment datums. Each narrowed cream stator stays inside the empty opening of an orange moving C-yoke through the full prescribed stroke. The exact validation requires three connected occupied yoke bars, a centered empty-cavity port, at least 0.4 world units of running clearance on every transverse stator face, and zero positive-volume overlap between the swept bars and stator. The bridge supplies no Rapier body, captive guide, solved load transfer, or stress evidence. Its cabinet, overhead bus, fixed housings, load beam, and stators form one exact face-connected external actuation route ending at fixed-stator/moving-yoke engagement; each moving head instead carries a precharged local buffer whose internal conduit and ram backing terminate at the pickup plate. The fixture simulates no charging, flexible moving feed, electricity, electromagnetic force, motor torque, feedback dynamics, or energy use. Both heads start preloaded: each component outer face exactly meets an energized pickup plate and a fixed joint retains it. After position, orientation, relative-speed, dwell, and two-voxel clearance insertion pass, the cap crown underside must meet the core top plane after its key occupies core layers seven and eight. Immediately before the pickup joint is removed and exact component colliders are installed on the retained base, the consumer measures the live body against its canonical merged pose, records the actual world-space translation and shortest quaternion-angle correction, rejects correction above 0.025 world units or 0.03 radians, and rejects deepest contact-manifold penetration above the hashed 0.001-world-unit tolerance. Retention is an explicit software compound weld, not a solved lock, latch, magnetic-force calculation, articulated jaw, or in-trace grab. Fifty-eight exact kinematic slats form one bounded articulated loop around two synchronized drums, while four minimal collision-excluded hub-and-radial flags derive from the same hashed phase without claiming gear teeth, contact, or torque. Exact OBB/SAT regression proves boundary-only end-cheek contact, central-barrel clearance, and detection of an overlapping negative control. Rapier contact and friction transport an axis-constrained carrier, and zero-drive and zero-friction ablations bound that causal claim. Full output pose and speed validation precede a prescribed no-snap rotation about the visible carrier trunnion. The axle extends beyond both belt edges into two open C-bearing cradles whose plinths face-contact separate occupied foundation guard-top solids. Each bearing remains at least 0.5 world units outside the belt; all non-axle carrier solids are axially separated from every dock solid; the dock neither penetrates the foundation nor overlaps the bucket; and the axle's complete canonical quarter-turn swept cylinder retains about 0.1528 world units of clearance. A third foundation-contacting foot supports an outboard servo housing whose safety coupler face-contacts the axle. Analytic sin/cos extrema check the complete configured-angle carrier sweep rather than sampled poses: the canonical carrier retains about 0.7528 world units from the foundation and 0.6000 from the bucket. The accepted live release pose repeats the continuous quaternion-aware sweep with at least 0.14 world units from the dock, about 0.7518 from the foundation, and about 0.5990 from the bucket before actuation; the measured tick, pivot, rotation, configured tip angle, radius, all three minimum clearances, and every limiting solid enter the final hash. The dock remains outside Rapier, so this proves neither a revolute constraint, bearing contact response, motor torque, feedback dynamics, nor energy use. The 30-second, 1,800-frame trace records 71 poses per frame and presents 74 instances, with assembly, release, contact, and collection evidence at about 11.67, 18.33, 20.87, and 23.87 seconds. Rapier is not imported by `src/`, emitted into `dist`, or made authoritative by Studio.

The headless `fixtures/windmill-consumer` proof is the second consumer of the extracted exact-sidecar Rapier adapter. Four ordinary Studio recipes become four solver bodies: one grounded frame with visible load paths to two separated rotor bearing rings and one hammer bearing ring, one dynamic rotor with a continuous shaft and two opposed stepped pitched plates, one dynamic follower-beam-head hammer, and one direct-ground fixed anvil. The three voxel rings communicate the solver-owned ideal revolute datums, cross-shaped journal clearances, and fixed visual routes to ground; the paired moving collars are bilaterally balanced visible shoulders whose equal opposite arms cancel their collar-subassembly radial first moments. Neither feature supplies solved bearing contact, bearing load sharing, friction, or an axial stop. The follower elbow is derived as the first grid column beyond the maximum bounded cam-nose extent, so an earlier elbow enters the contact envelope and a later elbow adds raised cells after that envelope ends. The consumer joins exact frame/body ports with two passive Rapier impulse revolutes; linked-body contact stays disabled, while collision groups admit only either exact cam nose against the follower and the hammer toe against the anvil face. A fixed 10 m/s world flow acts through the two-sided quasi-steady equivalent pitched-plate law `F = 0.5 rho Cd A dot(u-v,n) abs(dot(u-v,n)) n`, where each plate's area, centroid, radial/chord frame, and normal are derived from the exact occupied-cell union and `v` is the body's velocity at that centroid. The force therefore reverses with relative normal flow and naturally falls to zero at zero normal slip; no motor, one-way limiter, speed controller, startup ramp, pose script, velocity override, or post-step projection advances either body. At the repository's one 60 Hz solver rate both exact opposed cam noses contribute separately attributed contacts and qualified cycles over the nominal run, gravity returns the lever, and a positive contact impulse at the anvil completes each accepted ordered cycle. The nine qualified cycles occur in mixed nose groups, five to the primary nose and four to the opposed one, not strict alternation, and the two geometric cam passes per rotor revolution are opportunities rather than a claimed output cadence. The frozen bounded search evaluates all 144 candidates at both the short and full horizons and selects the first of 16 passes by minimum total occupied voxels, then dynamic occupied voxels, envelope volume, and stable key. Every visible purpose box has one exact sidecar peer, one unique evidence-record identity, one exact-box removal browser review, and one structural relocation diagnostic; a separately authored interface grammar rejects missing or unexpected same-body adjacency. The selected catalog ledger binds every removal and a bounded representative relocation set to declared fixed quarter cameras, while the generic preselection generator remains explicitly unbound. The fixture gates all collision-excluded visible pairs at every tick, anchor separation, out-of-plane pose drift, axis tilt and pose-derived axis rate, intentional-contact penetration, visible/contact parity, angular and tip speed, input work, and one-sided unaccounted-positive-energy diagnostics. Counterfactual runs separately disable wind, gravity, all cam contact, each individual nose's follower-contact participation, or post-lift anvil contact, and one further run removes one exact sail including its geometry, colliders, mass, and load. Each disabled-nose run removes that nose's attributed contacts and cycles, leaves every remaining event attributed to the other nose, and strictly lowers total completed cycles. The remaining nose's qualified-cycle count changes from four to seven when the primary nose is disabled and from five to six when the opposed nose is disabled, and both single-nose runs still exceed the three-cycle output minimum; the intervention establishes nominal participation, output sensitivity, and the declared dual-nose acceptance condition, not unchanged or additive attribution or either nose's necessity for minimum throughput. Delayed anvil disable preserves cam-driven lift but eliminates every later impact and qualified cycle; exact sail removal reduces rotor mass and breaks the paired bending and radial-mass balance. This is a bounded proof of quasi-steady relative-flow load, rigid-body inertia, ideal revolute constraints, gravity, contact, and open-system work input; the fixture also uses configured friction, restitution, and damping without independently isolating those parameters. It is not computational fluid dynamics, a pressure or wake field, turbulence, stall history, blade efficiency, bearing load sharing or friction, stress, deformation, fatigue, wear, heat, sound, forging, safety, arbitrary-wind performance, two-sided energy closure, or global energy conservation.

Around that fixture, the live Windmill scene shows the whole production line through authored presentation keyed to the impacts its browser solve actually lands. A mill building shell — two built walls on four corner posts under a stepped gabled roof, with a header beam spanning each otherwise-open east and south face and a stepped gable closing the west end to the ridge — houses the working bay while the rotor and sails stay outside the rotor wall; that wall carries one authored shaft opening with 0.198 world units of clearance around the shaft's swept cylinder and one base notch around the frame's ground tie. Every wall, post, roof, bin, and prop position is also gated against the committed fixture's rotor swept bands and per-frame hammer envelope by `windmill-production-clearance.test.ts`. Five wheat sacks queue as a finite visible infeed magazine, one is assigned to each blow the magazine can answer and follows an authored delivery-and-tip path, and a flour level in the outfeed bin rises one fixed step per sack milled. Two observed blows establish the live beat used to stage the next predicted answer; those startup observations do not prove that their assigned sacks were visibly delivered before impact. Only that next answer is staged, so a missed predicted blow leaves one sack waiting at the milling spot and advances no later sack. In the aligned live product-path run, the first six post-lift impact rising edges occur at ticks 110, 244, 382, 520, 658, and 796 at 60 Hz; their 2.233-to-2.300-second gaps all exceed `WINDMILL_SACK_SPOT_SECONDS_V1` at 1.270833 seconds, so each of the first five landed blows consumes one sack and later blows go unanswered after the magazine empties. The separate committed determinism replay has shorter sub-occupancy gaps and therefore skips intervening blows through the same filter; its six authored wheat-and-flour tracks are appended at codegen time, disclosed by the `authored-grain-flour-presentation` capability label and generated presentation manifest, and remain outside the four solver tracks' final hash. That replay is evidence, not the live scene's source of motion. The purpose graph declares the matching boundary: Windmill is open in `grain-mass` through the visible wheat infeed with no sink, flour and spent sacks accumulate inside, and the wheat-to-flour transformation at the anvil is internal authored presentation. Both lanes prove wind, rotor, cam, hammer, and anvil dynamics and nothing about milling, grain, or flour.

The private `studio.scene/4` pose-replay lane validates and defensively owns an explicitly supplied trace once, samples supplied poses at injected time, and projects them into ordinary sparse `patch-batch-instances` deltas. Cyclic V1 observations wrap only at their declared reset boundary; finite V2 observations clamp and hold their terminal pose until explicit Play or Space restarts at zero. The committed Windmill trace exercises that seam as determinism evidence, but the Windmill shelf scene uses its live profile and does not resolve the trace as its source of truth. The reusable seam is deliberately renderer-neutral: deterministic recipes define visible geometry, physical sidecars define consumer-readable physical meaning, a fixture or game owns solver rules and any V4 trace, and Voxel observes versioned plain-data poses through its existing delta path. Voxel still performs no integration, collision response, drive control, attachment decision, or feedback from presented animation to the fixture. This proves that bounded consumer-to-renderer seam, not a reusable simulation package, arbitrary-world physics, rollback, persistence, picking parity, or every item in the required-evidence list below.
...
Riverfall currently uses a fixture-local deterministic two-dimensional position-based-fluid thin-sheet solver. It advances 576 half-mass particles with projected gravity, fixed-order density corrections, XSPH smoothing, explicit boundary projection, heuristic lip and impact transitions, a speed cap, and an external recirculation pump. Studio restores a generated canonical post-burn-in state and advances every later state live on the shared 60 Hz lane; a separate byte-pinned determinism trace records 240 observed frames and never drives the shelf scene. A deterministic particle-to-grid presentation maps the current solver state onto 321 Eulerian tiles whose grid is checked against the live river, lip, exposed fall, pond, and outflow recipes: every cell-frame fails closed unless at least two visible particles lie inside a radius-10 world-Euclidean Wendland-C2 kernel, and only the nearest eight contribute its recording-start carried tracer, local-speed sample, and support-occupancy proxy. A hashed authored downstream coordinate adds a coherent carrier whose phase uses a bounded five-unit surface-wave floor plus local PBF-speed modulation, cross-reach neighbors smooth the scalar once, and fixed-orientation tile centers move only along the local surface normal so their footprints remain bank-contained. A 24-frame cubic Hermite presentation bridge and appended frame-zero pose close only the determinism replay without a pose discontinuity. The derived surface input hash covers the particle input, presentation configuration, and complete canonical topology, and paired near-versus-distant perturbation tests prove local influence without distant extrapolation. Every tile and the concealed underfill use the same blue. The carrier, tracer, occupancy proxy, displacement, neighbor smoothing, and replay bridge are presentation constructs rather than a solved water height, energy, or density field; they do not collide with rendered voxel geometry, and this is not a volumetric or free-surface Navier-Stokes engine.
...
Three fixtures were already doing this without a shared name for it. Riverfall runs an external recirculation pump. Windmill applies a fixed 10 m/s world flow, and its scene now also declares a visible grain-mass source: a finite five-sack wheat magazine enters at the infeed and nothing leaves, so flour accumulates inside the open system. Machine Works actuates from a precharged local buffer whose charging it explicitly does not simulate. Each is a point where the system opens, and each is why those fixtures may claim bounded work input but not conservation.
...
4. Spike Rapier behind a narrow adapter for dynamic and compound bodies, gravity, contact, friction, sleeping, and continuous collision detection. Machine Works and Windmill now share the extracted exact-sidecar adapter; repeated resource-stability and failure-injection teardown proof, a public or production consumer adapter, and browser lifecycle/performance proof remain.
5. Add fixed, revolute, and prismatic constraints. Use a hinged door, sliding drawer, and four-wheel serving cart as generic fixtures. Machine Works proves fixed joints and Windmill independently proves two consumer-composed revolute joints; a reusable generic constraint compiler and the prismatic fixture remain.
