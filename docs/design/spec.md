...
The sixth curated Studio proof scene, **Riverfall canyon**, proves a different use of the same generic presentation seam. Seven rebuildable opaque-voxel recipes define one connected landscape, river surface, waterfall curtain, receiving pond, outflow, flow marker, and foam cue; ten reused seed-varying tree placements line both sides of the high channel and lower pond banks. A private generic polyline sampler records 24 identical glint placements at constant arc length around one closed path within a six-second replay. Its visible reach runs downstream through river, fall, pond, and outflow; its return sinks inside the outflow, stays below the opaque foundation, and rises inside the source bed. The replay stores 600 frames at 10 ms, repeats frame zero as its final recorded pose, and uses the existing V1 held-reset behavior for only that final 10 ms, so the reset introduces neither a spatial pop nor a replay-schema change. Input and output digests, frame-zero fallback parity, outgoing corner velocity, per-step and reset displacement, exact reach alignment, path direction, hidden sink/underground/rise bounds, scene relationships, elevations, bank clearance, static non-overlap, default, overhead, longitudinal, and reverse fixed-camera captures plus a distinct flow phase, and exact 43-instance, 17-draw-call, 39,668-triangle lit game-look metrics are pinned. Studio uses RGB-only opaque recipe materials here, and the producer labels the replay as authored kinematic path sampling with zero gravity, so the proof claims visible one-way flow choreography rather than transparency, fluid dynamics, hydrology, or gameplay water state. Downstream consumers can adapt the same plain path-to-pose technique by sampling their authoritative state into ordinary public instance-matrix deltas; no Riverfall branch exists in the renderer or frozen public package API.
