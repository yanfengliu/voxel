...
The separate fifth **Machine Works** system scene contains only a rail, an explicitly non-colliding continuous-side-guide service gantry, a colliding tip carriage, two insertion stations, a three-piece signal module, and a collection bucket; the eight unrelated promoted industrial specimens remain in an explicitly named **Mechanical studies** contact sheet. The gantry rests on the rail foundation and keeps both externally prescribed kinematic heads visually aligned through their full strokes, but it is not a solver support. A fixture-local adapter maps the same exact authoring sidecars shown by Studio into Rapier `0.19.3` shapes, materials, CCD, sensors, and named ports. The consumer validates port position, orientation, relative speed, and dwell before compound attachment, drives every visible carriage pose through its retained physical body, generates 1,080 fixed-step frames over 18 seconds, records velocities plus assembled, released, positive contact-manifold, and collected events, and fails unless the product settles inside the declared bucket sensor within the hashed 0.05-unit solver tolerance. Private read-only `studio.scene/4` defensively copies the committed hash-bearing trace and projects sampled poses into ordinary renderer instance deltas with procedural placement animation neutralized; neither the authoring schema nor Voxel integrates physics, decides attachment, or feeds displayed animation back into the trace.
