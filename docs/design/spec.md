...
The separate fifth **Machine Works** system scene contains ten independent rebuildable assembly-line recipes, an explicitly non-colliding continuous-side-guide service gantry reused from the contrast catalog, two insertion stations, a three-piece signal module, and a collection bucket; the eight unrelated promoted industrial specimens remain in an explicitly named **Mechanical studies** contact sheet. The gantry rests on the conveyor foundation and keeps both externally prescribed kinematic heads visually aligned through their full strokes, but it is not a solver support. Fifty-eight exact 26-voxel-deep kinematic slat bodies form a closely pitched articulated conveyor loop around two internal drive-drum bodies; each underside follows the nominal 2.75-unit drum pitch datum and straight and turn clearances are explicitly bounded. An exact sidecar OBB/SAT regression samples every slat against both pitch drums over 32 phases, requires slat-edge and drum-end-cheek compounds to share boundary only with zero positive-volume overlap, verifies at least about 0.275 world units of central-barrel radial clearance, and detects an intentionally overlapping negative control. Four exposed axle cogs reuse the tenth exact cataloged kinematic recipe and sidecar but never enter Rapier; the consumer derives their four non-interacting replay tracks from the solved drive-drum poses, so all 64 presented slat, drive-drum, and exposed-cog elements derive pose and speed from one hashed drive phase while the cogs prove only visible axle-phase agreement. A fixture-local adapter maps the nine causal authoring sidecars shown by Studio into Rapier `0.19.3` shapes, materials, CCD, sensors, and named ports. Contact and friction between the moving slat colliders and an axis-constrained dynamic carrier transport the retained carrier body through two station targets without a prescribed carrier X trajectory; the carrier's locked axes align with visible foundation guards but the proof claims neither physical side-guide contact nor exposed-cog contact. Two same-geometry 240-tick ablations preserve the complete causal world of foundation, slats, drive drums, carrier, jointed product base, stable body order, controller, and gravity while removing one named cause: zero drive must keep maximum displacement within 0.05 world units, and driven zero friction must keep maximum displacement within 20 percent of the driven trace's maximum over the same interval. The consumer validates port position, orientation, relative speed, and dwell before compound attachment, then validates the carrier's full X, Y, Z, orientation, and speed state at output; the explicit position-based tip servo captures that exact accepted dynamic pose and begins there without a positional or rotational snap. The resulting 30-second trace records 71 pose tracks over 1,800 fixed-step frames and presents them with two static supports as 73 scene instances, including assembled, released, positive contact-manifold, and collected events at about 11.67, 18.33, 20.95, and 24.15 seconds; collection still requires the product to settle inside the declared bucket sensor within the hashed 0.05-unit solver tolerance, and the final frame holds until the one discrete reset at exactly 30 seconds. Private read-only `studio.scene/4` defensively copies the committed hash-bearing trace and projects sampled poses through ordinary sparse instance deltas with procedural placement animation neutralized, so downstream consumers can reuse the recipe-sidecar-trace boundary without a renderer branch. Neither the authoring schema nor Voxel integrates physics, decides attachment, or feeds displayed animation back into the trace, and this proof makes no claim about exposed-cog contact, cog torque transmission, belt tension or compliance, tooth engagement, or arbitrary-load no-slip behavior.

The sixth curated Studio proof scene, **Riverfall canyon**, uses the consumer-owned trace seam without making simulation renderer state. Seven rebuildable opaque-voxel recipes and ten seeded tree placements remain ordinary static scene geometry, while the fixture-local `studio.riverfall-fluid-domain/1` sidecar partitions one continuous 142-unit path into exactly twelve reaches: river, lip, fall, pond expansion, pond basin, pond contraction, outflow, visible submergence, hidden sink, hidden under-foundation return, hidden source rise, and visible emergence. A deterministic 2D PBF thin-sheet/surface solver advances 288 fixed-mass internal particles and records 600 genuine frames at 10 ms after two fixed 5 ms substeps per frame; 96 stable particle witnesses become the generated replay attached to private `studio.scene/4`, with solver identity, timestep, gravity, law and capability labels, and input and final hashes. Projected gravity acts along the domain tangent, a fixed-order Jacobi density constraint and XSPH couple neighboring particles, explicit lip attachment turns the sheet downward, dissipative fall-to-pond impact feeds the widening basin, sloped-bank contact projects and reflects along the physical strip normal, and an occluded external pump recirculates particles through the sink, return, and source rise between the two visible transitions. Studio validates and defensively copies the replay, neutralizes procedural placement animation for those tracks, and presents sampled poses through ordinary sparse instance deltas. Frame 599 remains a genuine solver result rather than a copy of frame zero, so it is held from 5,990 ms until the discrete reset at 6,000 ms instead of fabricating a periodic endpoint. Fixture tests hold the remaining world fixed while separately ablating projected gravity, hidden-pump forcing, the density solve, and XSPH. Fixed browser evidence covers opening and evolved default phases, overhead, longitudinal, reverse held-final, and reverse reset views while pinning 115 instances, 17 draw calls, 40,148 triangles, stable 17-batch geometry/material resources, and two renderer textures across repeated exact draws. The opaque voxel recipe geometry does not deform, and this bounded proof claims neither volumetric or free-surface Navier-Stokes, transparent or refractive water, erosion or hydrology, nor gameplay water state; no Riverfall branch exists in the renderer or frozen public package API.
