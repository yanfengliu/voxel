...
Iteration 2 reviewer verdict (2026-07-30): both verification lenses read the four iteration-1 commits against live code and found every claimed fix holding — capture ownership, the lifecycle fence, the colour stride, the cell hash, the moving-vs-moving lane's dedup and cross-bucket handling, the intrinsic copies, the exact remainder, and the gate fixes — with two exceptions they were right to press on, plus four smaller residuals. All are now closed: the batch-lane gap (below), a Linux CI regression the license sweep introduced (`a6ec414`), and three residuals in `a6ec414`'s tree — a chunk validating its input with the caller's own overrideable `length`, an announcer that never printed the pairs it could not judge, and a request writer that an origin check alone cannot protect from DNS rebinding. Codex's data-plane lens tripped its cybersecurity filter for the third consecutive run and never wrote a review, but its last probe line — `validated true { length: 0, bytes: 0 }` — exposed a mesher input whose sample volume a later accessor detaches mid-validation, fixed in `3ecf243`.

A bookkeeping note kept honest rather than rewritten: `a6ec414`'s message describes only the license-sweep fix, but the commit also carries those three residual fixes. `git add` had staged them for an isolated-worktree check and the commit that followed took the whole index — the pathspec habit exists for exactly this, and it slipped. The split was already pushed by the time it was noticed, and rewriting published history beside an active concurrent session is not worth a tidier log, so the record is corrected here instead.

Iteration 2, first result (2026-07-30, commit 9dd15b8): the F13 fix was incomplete and the driver found it before the reviewers reported. Resources and chunks take ownership through `ownParsedTargets`; batches do not — their paged plans are built further down, still after every caller accessor has run — so the window stayed open on exactly the lanes the fix did not touch. A probe landed a NaN matrix element in canonical state through both an accepted `put-batch` and an accepted `patch-batch-instances`. Both shapes now revalidate and take ownership before any plan is built, the transaction is refused naming the exact element, and the two regression tests fail with the pass disabled. The lesson generalizes: a fix that closes three lanes out of four reads exactly like a fix that closes all four, so enumerate the lanes and probe each one rather than reasoning from the shape of the patch. Reviewer verification of the remaining items is still in flight; Claude's Fable quota ran out mid-iteration and those two lenses fell back to Opus, per the runbook.
