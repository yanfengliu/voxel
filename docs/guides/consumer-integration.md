...
AoE2 currently uses chunks for terrain and consumer-authored geometry plus rigid instance batches for entities and feedback. Its AoE-owned `AoeVoxelGameView` is the sole world-renderer host: it owns camera/input orchestration and projects fog, selection, placement, health, hit, and death feedback into the same Three/`voxel` presentation. The DOM HUD and minimap remain separate product UI; there is no Phaser source, dependency, renderer selector, fallback, or second world canvas. City draws its building wall lane through an embedded, borrowed-renderer Voxel runtime while retaining its terrain and composition root; further lanes adopt the same pattern. Townscaper should first align to the tested Three release, then pass its locally generated connected shell through geometry resources; its massing and facade rules stay local.
...
- The visible-face mesher remains the correctness oracle. The corpus-proven in-repo greedy mesher was accepted for production on 2026-07-15 ([selection ADR](../architecture/mesher-selection.md)) and runs in the packaged worker through the revision-atomic runtime path.
...
- Whole snapshots, additive atomic deltas, the packaged worker, the bounded stale-safe scheduler, revision-atomic async runtime staging, and committed-store picking are implemented: `voxelWorkers` is a public runtime option, and `pickPresented` answers bounded queries from the exact presented frame. Assets and spatial sharding remain later contracts and must preserve accepted/presented revision semantics. The portable dense-chunk ray query remains independent of runtime state.
