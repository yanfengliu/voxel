...
The sixth station is the first whole machine, and it exists to exercise the one capability no free-body station touches: joints. `studio:scene:physics-trebuchet` builds a staked trestle frame, a 3:1 arm whose drawn rods are where every joint anchors, a hinged crate of steel at 990.6 mass (twenty-eight times the ball), a rigid-link sling with an open-fronted pouch, a stone ball, a trigger post, a target wall, and a catch berm past it. Four constraints join them — the axle (revolute), the counterweight hinge (revolute), the sling pivot (revolute, because a drawn hook on a crossbar is a hinge and a real sling's two cords keep the whip planar), and the trigger lashing (a rope).

**fire** detaches the lashing and nothing else: the crate falls, the arm whips the sling, and the ball leaves the open pouch front when the whip swings past it — release is geometry, not script, because no action ever touches the ball. Measured on the reference run: the ball leaves the sling at 15.8 m/s, arcs to 14.9 m, and comes back down through the wall's upper-middle course 32 m away, still doing 12.2 m/s when the first brick moves at tick 974. Six scenarios pin it — `treb-hold` (three cocked seconds, everything still), `treb-fire`, and four controls that each remove one part and assert what breaks: no counterweight (the fired arm sweeps 0.4°), no sling (the arm swings, the ball moves 42 cm), and no berm (the ball rolls off the field and the run fails).

The wall is what makes the throw legible, and it needs no destruction system: it is 33 separate stone bodies — 27 bricks and 6 half-brick closers — stacked six courses high in a running bond and bonded by nothing at all. It comes apart on impact because the pieces were only ever resting on each other. Measured, it stands through the three-second hold with every brick inside 0.02 m, and the shot moves 30 of its 33 bricks more than 0.25 m, the farthest by 3.31 m; the base course barely shifts, so the shot takes the wall off its footing rather than sweeping the footing away. Four named bricks above the base carry that as checks, because a glancing top-clip would leave exactly those standing.
...
10. **Nothing stops a rolling ball.** With no rolling resistance the landed ball held a constant 4 m/s and left the last floor tile around tick 2100. The catch berm is what ends the run on screen, and `treb-fire-no-berm` is the executable proof. It is also why the ball, rebounding off the wall, rolls 26 m back up the field before it meets the machine.

Findings 1 to 4 and 7 came from the deterministic scenarios or the one-step spawn-penetration gate; 5 and 6 came from a headless parameter sweep run specifically to make the machine hit harder; 8, 9 and 10 were caught by adversarial review measuring what the scenarios did not.
