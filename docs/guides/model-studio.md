...
The current renderer is an internal Studio-only WebGL2 forward+ proof. It bins visible lights into 48-by-48-drawing-buffer-pixel tiles and 24 depth slices, stores light records and cluster indices in data textures, and runs a fixed shader loop of at most 32 local light evaluations per fragment, so authoring or moving more lights does not create a shader variant per count. The 4,096 authored-light ceiling is not permission for 4,096 lights to overlap one fragment: preparation rejects a cluster above 32 influences, excessive cluster work, excess unbounded-range lights, or an oversized viewport with a specific error and retains the previously prepared lighting instead of silently dropping an influence. All visible light handles share one `InstancedMesh`, so they add one marker draw call rather than one object and draw call per light.

The built-in **1,000 orbiting lights** scene (`studio:scene:lighting-1000`) is the deterministic correctness and performance showcase: it distributes one thousand independently moving point lights over ten depth bands around repeated sandstone fixtures. It demonstrates immediate raster point lighting only—there is no ray tracing, bounce lighting, denoising, reflection, or shadow path.

A sealed [named-hardware run](../../benchmarks/results/2026-07-26T05-12-53-840Z-clustered-lights-1000.json) measured commit `1f4377d` in headless Chromium 147 on an RTX 4090 at 1280×720 DPR 1. Five identical 600-frame sequences produced 1,201.2 GPU-synchronized uncapped Studio draws/s, 0.80/0.90/1.10 ms p50/p95/p99 frame time, and an 826.4 draws/s slowest-1%-mean rate; all 1,000 lights stayed visible and moving, cluster occupancy stayed at 13 of the 32-light fragment budget, the 1,000→999→1,000 probe compiled no new programs, and 35 receiver placements plus all markers drew in 2 calls and 41,680 triangles. These are `drawAt` plus `gl.finish` microbenchmark numbers for CPU cluster preparation, uploads, and a small receiver batch—not requestAnimationFrame/display FPS, gameplay FPS, or proof that a full game-sized scene sustains the same rate.
