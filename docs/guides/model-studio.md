...
- the left rail switches between **Models**, **Parts**, **Recipes**, and — when the catalog ships any — **Scenes**, over one search box; every item is a flat name-only button that renders or opens on click, model section headings are fixed rather than expandable, and descriptive metadata appears in the right **Examine** inspector; drag an unfiltered entry to rearrange it, or use **Move up** / **Move down** in the same keyboard-navigable **⋯**, right-click, and `Shift`+`F10` action menu; models remain inside their catalog section;
...
When a catalog ships scenes, the rail grows a **Scenes** lane; opening a scene draws the whole arrangement on the stage under the same look controls, and opening any model leaves it. Drag a scene to rearrange the lane, or use its **⋯** button, right-click the row, or press `Shift`+`F10` while it is focused to move, rename, or delete it. Renaming an open editable scene participates in its undo history; deleting any open scene, including a read-only replay scene, returns to the underlying model and clears its selection and history.
...
Open **Scenes → Machine works**, turn **animation enabled** on, and press **Play**. The 18-second replay window starts with a product base on the rail-guided transfer carriage, lowers the core at the first insertion head, lowers the cap at the second, completes the three-piece signal module at 9.0 seconds after both named mating checks, reaches the bucket and releases its carriage joint at 12.0 seconds, visibly tips the still-colliding carriage around its servo hinge, contacts the bucket at the solver-recorded 13.52-second manifold, and satisfies the declared-sensor inside-and-slow collection check with its hashed 0.05-unit solver tolerance at about 16.63 seconds. The final recorded frame remains held through the end of the window; reaching 18.0 seconds performs one discrete reset to the first frame instead of interpolating the product backward across the factory. Pause or scrub the shared timeline to inspect any exact phase; the lighting toggle changes only illumination.
...
The **Machine Works** model shelf contains the rail foundation, bucket, carriage, insertion head, base, core, and cap as independent rebuildable recipes. Open one there and enable **colliders** to inspect the sidecar's compound solid/sensor shapes and attachment ports; the bucket alone shows its interior collection sensor. The source sidecars additionally declare density, friction, restitution, and the base product's CCD request for the fixture adapter, but the current overlay does not display those numeric properties. These sidecars remain authoring data and do not run a solver.

The replay is generated headlessly by `fixtures/machine-works-consumer/machine-works-simulation.ts` using pinned `@dimforge/rapier3d-compat` `0.19.3`, then committed as hash-bearing Float32 observations. Its input hash covers the sidecars, grains, stable creation order, trajectories, joint ports, contact flags, material values, mating tolerances, merge rule, servo tip, CCD request, and collection thresholds; code generation derives solver, gravity, law, capability, and hash claims from the trace. It covers fixed-step rigid-body gravity, exact compound primitive contact, friction, restitution, continuous collision detection on the released product, kinematic actuation, validated fixed-joint-to-compound assembly, and declared-sensor bucket containment with the hashed 0.05-unit solver tolerance. It does not prove arbitrary machinery, deformable bodies, fluids, thermodynamics, energy conservation across every solver approximation, or “all physics laws,” and Voxel never derives a physical outcome from what it renders.

The current renderer is an internal Studio-only WebGL2 forward+ proof. It bins visible lights into 48-by-48-drawing-buffer-pixel tiles and 24 depth slices, stores light records and cluster indices in data textures, and runs a fixed shader loop of at most 32 local light evaluations per fragment, so authoring or moving more lights does not create a shader variant per count. The 4,096 authored-light ceiling is not permission for 4,096 lights to overlap one fragment: preparation rejects a cluster above 32 influences, excessive cluster work, excess unbounded-range lights, or an oversized viewport with a specific error and retains the previously prepared lighting instead of silently dropping an influence. While perspective lighting is active with more than 32 effective finite-range sources and no unbounded source, camera gestures keep the eye outside the scene's complete static or moving finite influence volume, bound pitch short of an edge-on projection, and pin ground-plane panning to the scene origin; this data-derived distance plus centered safety policy prevents the built-in dense proof's reachable camera views from conservatively collapsing its source cloud into one cluster. Flat and unlit views retain the ordinary camera range and unrestricted pan, and re-enabling perspective lighting reapplies pan, pitch, and zoom transactionally before drawing. A scene with unbounded sources or a larger or more overlapping authored field remains governed by the explicit cluster and work limits, which preserve prior lighting whenever its prepared frame would overflow. All light handles share one `InstancedMesh`, so they add one marker draw call rather than one object and draw call per light; disabling lighting changes that shared batch to a subdued edit-handle presentation without allocating another material or program.
