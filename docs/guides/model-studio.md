...
#### Riverfall visual-flow proof

Open **Scenes → Riverfall canyon**, turn **animation enabled** on, and press **Play**. The default front-left camera shows a high river between asymmetric seeded tree rows, a blue curtain dropping between cliff shoulders, a lower pond with four breathing foam patches, and a narrow outflow through the front bank. Turn **lighting on** for shaded cliff and tree faces, switch between study edges and game look, scrub the six-second window to inspect the cyan glints at exact phases, and orbit to the front longitudinal view to read the complete source-to-outflow chain without relying on the scene title.

The landscape, river, waterfall, pond, outflow, glint, and foam are seven independent shelf recipes; the ten trees reuse the ordinary seed-varying tree recipe. Twenty-four instances of one glint recipe follow a hash-pinned constant-arc-length polyline. The visible part runs downstream across the river, over the lip, down the fall, through the pond, and into the outflow. The loop then sinks through opaque geometry, returns below the foundation, and rises inside the source bed, avoiding an unsupported reverse stroke or fade. Its 600-frame trace records frame zero again at frame 599, so V1's ordinary final-frame hold lasts only 10 ms and the six-second reset cannot pop to a different pose. The same scene remains structurally legible with animation disabled.

This is authored visual choreography, not simulated water. Studio recipe palettes are RGB-only and compile to opaque materials; the pose replay moves rigid placements and cannot deform, fade, refract, change topology, or conserve water volume. Its provenance therefore records zero gravity and kinematic path labels rather than fluid-law claims. A downstream game can keep the reusable connected recipes and path vocabulary while replacing the authored marker replay with authoritative water-state matrix deltas or its own continuous geometry and constant material transparency. Voxel still receives ordinary geometry, materials, instances, and revisions; there is no Riverfall renderer branch.
