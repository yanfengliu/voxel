...
`studio.scene/2` scenes may also carry up to eight plain-data point lights with stable ids, positions, sRGB colors, intensity, and range; adding the first light to a V1 working scene migrates that edited copy to V2. An older Studio therefore rejects a lit scene instead of accepting V1 and silently omitting its behavior. The Edit tab adds, moves, recolors, brightens, dims, and removes lights through the same whole-scene undo history, while small colored handles make their positions visible. This is immediate Three.js raster lighting on the Studio's Lambert materials: it does not claim ray tracing, bounce lighting, denoising, reflections, or shadows.
