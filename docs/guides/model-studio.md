...
The **lighting on/off** toggle is one global stage-view preference shared by models and scenes, not a property of the open scene. Its visible text, pressed state, scene status, stage hint, and right-side scene details all report the active state. Switching it off keeps lighting off when another scene or model opens and after a reload through the Studio's injected view store, backed by guarded `localStorage` by default; disabled source handles remain visible but dim for editing, contribute no raster light, do not orbit during ordinary playback, and do not keep a light-only scene's player running. When browser storage is unavailable, the Studio remains usable but falls back to the default on the next visit.
