...
A scene with a live-physics profile — the chain link study, the ball drop, and the nine physics-playground scenes — adds **adjust** / **interact** stage buttons and opens in Interact, because testing the physics by hand is what such a scene is for. Playground scenes additionally show the playground panel (transport, scripted cases, spawn/remove/impulse, ramp angle, and a per-body debug readout with a collider/contact overlay); [the physics playground guide](physics-playground.md) covers the stations, their declared constants, and the findings they exposed. In Interact the left button belongs to a live Rapier solver whose bodies start at the recording's opening poses (the chain's rings spawn tilted along their catenary tangents, exactly as recorded, so the live chain threads and hangs instead of coming apart): dragging a moving part pulls it against a spring, releasing lets go, and on the ball drop a clean click under the rail releases the next ball. Nothing in this lane is recorded. Adjust returns the editing pointer — on the editable ball drop it selects and drag-moves models, while the recorded chain study stays read-only as above — and the stage hint names what the left button does in whichever mode is active.
