...
On Voxel's Studio dev server, **Send request** saves a timestamped JSON file under `tools/studio/requests/`; it starts no agent and sends no notification. Ask an agent to process the reported file when ready. An agent processing a model request renders the exact model the owner was looking at with `npm run studio:sheet -- <file>`; for a scene request, `npm run studio:request -- <file>` restores every pinned view through the studio's own capture contract — camera, look, phase, replay evidence, and stage size all verified — and writes each pin's picture, a closeup around the pinned spot, and a summary beside them under `output/studio/requests/`.
...
An ordinary scene is edited by arranging it, not by building steps, so it hides Build and Motion, keeps Notes available, and fills **Edit** with a scene editor: an add-model picker and a list of placements. Selecting one — in the list, or by clicking the model on the stage — opens its move, turn, and remove controls and outlines it. Selection is one thing the stage outline and the Edit controls share, so clicking a second model moves the controls to it. One pointer rule holds in every view, and it is worth stating once because nothing overrides it: the left button acts on what is under it, the middle button turns the camera, and the right button pans. Left never turns the camera — not on the model stage, not on a scene that poses itself, and not when a grab in Interact catches nothing. It used to fall back to an orbit whenever it missed, which made reaching for a swinging chain link the single most likely way to fling the view. A left press that finds nothing to act on now simply consumes its own drag. On the stage a left-drag slides the selected model across the floor, a middle-drag turns the view, a right-drag pans, the wheel zooms from 0.25 to 256 visible vertical world units, and held `W`/`A`/`S`/`D` moves the camera continuously across the ground relative to its current yaw; diagonal movement is normalized, and releasing the keys stops immediately. Opening an asset retains the established comfortable 3-to-80 auto-fit, after which the wheel can travel across the wider inspection range. Dense finite lit scenes use a conservative first view and then the same complete camera range and movement controls; an exact camera that exceeds the clustered-light limits is rejected with the prior view intact rather than pre-emptively clamped. The same WASD navigation works while a model or consumer replay is open, and does not run while focus is in a text field or control. Double-click restores both the default orbit and the world origin on the model stage only; in an open scene of any kind it never moves the camera, and its two clicks act as the ordinary clicks that scene already defines. **Snap to grid** (a scene-only toggle) lands a dragged model's footprint on whole cells. `Ctrl`/`Cmd`+`Z` undoes a scene edit; `Ctrl`+`Y` or `Ctrl`/`Cmd`+`Shift`+`Z` redoes it.

Every scene the studio opens or edits is checked for surface conflicts, because real objects have volumes and two things cannot co-exist in the same space: placements may rest flush against each other, but may never share volume, and a recorded pose may never put a surface on a still surface's plane facing the same way. Tilted recorded poses are judged for shared space by the exact box test (a tilted face cannot hold a plane, and the report says so), and a film placement is a drawn skin that claims no space. The Examine pane announces findings in plain words as soon as the check finishes — it runs after the scene presents, so opening stays instant; the first look at a heavy recorded scene then pauses one long frame while the check runs, and every later visit reads a cache. `window.voxelStudio.sceneSurfaceConflicts()` hands an agent the same lines, an edit that creates a conflict is announced immediately, and undoing it silences the line. The pinned scene tests hold the same rule for every built-in scene, so a conflict cannot land silently.

The scene **Notes** tab keeps one autosaved brief and an ordered queue of numbered annotations for each stable scene id. Choose **Annotate scene**, then use one left gesture on the picture: this explicit one-shot mode takes precedence over ordinary placement selection or dragging and over the left button's ordinary meaning in whichever view is open, freezes a moving scene at press time, places a bright `+` exactly where the press captured the picture, and opens the annotation editor on release. A gesture that becomes a drag restores its prior playback and stays armed rather than capturing the later release position. Queueing replaces the draft marker with the annotation number; canceling removes it. While the draft is open, the scene view, playback, look, shelf, editing tabs, and column resizing are presentation-locked, and an outer window resize temporarily stretches the unchanged captured raster instead of moving the target under the marker. A rejected capture restores the prior playback and armed state instead of leaving the scene paused. `Shift`+`Enter` adds a line, `Enter` queues the annotation, and `Escape` cancels it. Text fields retain native typing and undo behavior, so `W`/`A`/`S`/`D` and `Ctrl`/`Cmd`+`Z` do not move the camera or edit the scene while a brief or annotation has focus; covered model-only controls are inert and absent from the scene tab's keyboard and accessibility order.
...
**Send request** from a scene saves a private `studio.request/2` artifact containing the trimmed scene brief, only that scene's pins, a bounded snapshot of the current scene, and the current camera, center, selected placement, depth, lighting and edge look, unwrapped exact time, and replay provenance. It includes the V4 replay reference and hashes but never copies generated replay frames. Request filenames are created exclusively and collision retries never overwrite an earlier review; a delayed response also cannot replace status from newer same-scene brief or pin edits. Like the model request route, saving starts no agent and sends no notification; it adds no public `voxel/core`, `voxel/three`, physics, or simulation API. `npm run studio:request -- <file>` renders the saved pins back exactly as captured (and a scene that has drifted since the capture fails with the studio's stale reason instead of rendering something the owner never saw), so the file is evidence an agent can look at, not only data.
...
A `studio.scene/4` consumer replay is an observational, read-only scene in Studio. Its Edit tab explains that boundary instead of exposing authored placement or light controls; the snap toggle, stage picking, selection outline, and placement dragging are disabled because they describe the static source scene rather than the poses currently presented by the replay. A left drag does nothing at all — the same one pointer rule the rest of the Studio follows — while a middle-drag turns the camera, right-drag and WASD move the view, the view and lighting controls remain available, Play and timeline scrubbing remain available, and the scene can still be deleted. Harness selection, snap, rename-while-open, and scene-edit attempts reject with an actionable message; change the consumer simulation or trace source and regenerate the replay when the assembly itself must change.

A scene with a live-physics profile — the chain link study, the ball drop, the nine physics-playground scenes, and the windmill, machine works, and riverfall scenes — adds **adjust** / **interact** stage buttons and opens in Interact, because testing the physics by hand is what such a scene is for. Playground scenes additionally show the playground panel (transport, scripted cases, spawn/remove/impulse, ramp angle, and a per-body debug readout with a collider/contact overlay); [the physics playground guide](physics-playground.md) covers the stations, their declared constants, and the findings they exposed. In Interact the left button belongs to a live Rapier solver whose bodies start at poses the scene computes for itself (the chain's rings spawn tilted along their catenary tangents, which is the only reason the links thread instead of colliding — `chainLiveSpawnPosesV1` derives that curve analytically and reads no recording): dragging a moving part pulls it against a spring, releasing lets go, and on the ball drop a clean click under the rail releases the next ball. Nothing in this lane is recorded. A left press that catches no body turns nothing; the camera stays where it was, because the one pointer rule above holds here too. Adjust returns the editing pointer — on the editable ball drop it selects and drag-moves models, while the recorded chain study stays read-only as above — and the stage hint names what the left button does in whichever mode is active.
...
A scene that only a solver moves — Machine works, the chain link study, the ball drop, the nine playground stations — shows no scrubber and no speed control under the stage, because a scrubber offers a position in a recording and such a scene has none. The bar reads `live · N solver steps` instead, which is the number that does say where it has got to and the one its browser proofs settle against, and Play/Pause there starts and stops the solver. Riverfall keeps its scrubber: its water cannot be scrubbed, but its kelp and pond ripple are authored motion on a real period, so the timeline names a real place.

The separate **simulation on/off** button is a persisted scene-wide movement preference shared by every scene, and it governs every way a scene can move: authored model motion, moving point lights, consumer pose replays, and the live solver alike. That last one was missing until 2026-08-14, and the gap showed in two directions. Machine works owns no animated recipe and no light, so the switch that only knew about the scene clock concluded the scene could not move and hid itself — on the one scene in the catalog that is nothing but simulation. Riverfall showed the switch and left its water running when it was off. The button now appears whenever the open scene has authored motion, a moving light, or a live-physics profile, and turning it off holds the solver exactly where it stands: no step, no pose rebuild, no redraw, and the clock re-anchored so resuming does not hand the solver the whole pause to catch up on. On a motion-bearing scene, ordinary Play enables the preference and resumes moving point-light sources, animated model placements, and consumer pose replays from the presented phase, while Pause disables it and holds all three; the button reports that saved permission, and Play/Pause reports whether the transport is advancing right now. Bare `Space` invokes that same Play/Pause action when the active Studio owns page or stage focus; text fields and focused controls retain their native Space behavior, and holding the key toggles only once. Exact-time harness draws, timeline seeks, and rejected frames pause the transport transiently at the last phase they actually presented without rewriting an enabled preference, so Play or `Space` remains the explicit way to resume that inspection pause. A newly opened or reloaded animated scene honors the stored choice; a fresh Studio defaults to the simulation on, while a legacy stored view without the field migrates its former moving-light behavior from the stored lighting choice before subsequent writes become independent. When browser storage is unavailable, the Studio remains usable but falls back to the defaults on the next visit.

#### Wind-powered trip mill proof

Open **Scenes → Wind-powered trip mill**, turn the **simulation on**, and press **Play**. The scene contains four functional assemblies: the grounded frame carries two separated rotor bearings, one hammer bearing, and their foundation ties; the two stepped pitched sail plates collect wind load into one continuous shaft; both opposed cam noses contribute follower contacts and qualified cycles during the run; the hammer's upper head cell face-connects its right beam to its impact toe and contributes sidecar-derived static mass and analytical head-down gravity torque; and that toe strikes a direct-ground anvil face. The selected fixture's nine nominal cycles group five to the primary nose and four to the opposed nose, so neither strict alternation nor two completed trips per revolution is claimed. The voxel bearing rings show ideal-joint datums and fixed visual routes to ground, but do not contact the linked moving bodies in Rapier; the passive revolutes at named ports enforce the axes. The paired collars are balanced visible shoulders, not solved axial stops, and the follower elbow sits at the first column beyond the bounded maximum cam nose so neither interference nor a purposeless raised extension is accepted. This is not an isolated dynamic ablation of the upper cell: the H1/H2/H3 search varies multiple geometry and mass conditions and does not prove that cell independently necessary or responsible for a cycle. Bare `Space` pauses or resumes the live browser solve. The separate 12-second committed trace ends on an exact recorded terminal state and holds it thereafter, but it is determinism evidence rather than this shelf scene's source of motion. Pause, then orbit to the side and rear to verify that the shaft passes through both rotor bearings, the hammer journal passes through its bearing, each cam nose clears the follower after lift, and the toe meets the anvil instead of floating through it. The four corresponding **Windmill** shelf recipes remain independently rebuildable and expose the same exact named boxes, colliders, materials, masses, and ports consumed by the fixture. Their selected purpose ledger assigns a unique record to every exact box, binds canonical front/side and every exact-box removal to declared quarter-camera footprint and pixel-difference thresholds, and retains structural relocation evidence for every box plus visual relocation artifacts for the bounded representative set; static `studio.scene/3` review variants are mounted exactly as authored and do not borrow a replay pose.

The committed replay is generated headlessly by the consumer-owned `fixtures/windmill-consumer` Rapier `0.19.3` fixture at the repository's one 60 Hz solver rate and recorded from that same solver loop at 60 Hz: the opening state plus every solver tick produces 721 frames over 12 seconds of physics and 12,016.666… milliseconds of finite presentation. A fixed 10 m/s world-air design point drives the two exact plates using `F = 0.5 rho Cd A dot(u-v,n) abs(dot(u-v,n)) n`; the exact occupied cells determine each equivalent area, centroid, chord/radial frame, and normal, and the body's live velocity at that centroid supplies `v`. The force changes sign with relative normal flow and tends to zero with normal slip, without a motor, speed controller, ramp, scripted pose, velocity override, or post-step correction. Passive impulse revolutes constrain the rotor and hammer, collision filtering admits only the two individually named cam noses against the follower and the hammer toe against the anvil, and each accepted cycle requires attributed cam contact, minimum lift, release, apex, downward return speed, and a positive anvil impulse in order. The selected minimum-form candidate is the deterministic first of 16 passes after all 144 candidates run both the short and full horizons and completes nine nominal cycles, with both cam noses causally represented. Generation gates every collision-excluded visible pair at every solver tick, exact physical/visual parity, anchor and out-of-plane pose error, axis tilt and pose-derived axis rate, active-contact penetration, angular and tip speed, input work, and one-sided unaccounted-positive-energy diagnostics. Seven counterfactual runs separately disable wind, gravity, all cam contact, each individual nose's follower-contact participation, or post-lift anvil contact, or remove one complete sail with its geometry, colliders, mass, and load. This fixture implements a bounded quasi-steady relative-flow plate law and rigid-body mechanism; it does not solve computational fluid dynamics, pressure or wakes, turbulence, stall history, blade efficiency, bearing load sharing, deformation, fatigue, wear, heat, sound, forging, safety, arbitrary wind, or global energy conservation. The replay remains accepted plain-data determinism evidence; Studio's private live profile solves the shelf scene in the browser, while renderer/core owns neither simulation lane.

#### Riverfall live fluid proof

Open **Scenes → Riverfall canyon** and watch. The water is solved in your browser as it runs, so there is no timeline to scrub and no loop to wait for. Studio restores a generated canonical post-burn-in solver state so the filled closed loop can open without repeating the 3,200 warm-up substeps on the browser main thread; every later state is solved live. The default front-left camera shows a high river between asymmetric seeded tree rows, a blue curtain dropping between cliff shoulders, a lower pond, a narrow outflow through the front bank, one continuously covered blue surface whose voxel ripples travel across every reach, and 96 pale foam flecks riding the solver's own particles down it.

Those flecks arrived on 2026-08-14, and the scene refused a foam layer outright until then. What retired that position was a measurement: the sheet is one flat colour, so its only visible channel is its outline, and with the studio's default unlit look the water moved **0.75-1.6% of the stage's pixels per 200 ms** — quadrupling the tile displacement and tripling the tilt gain took that to 1.8-2.7% while the curtain stayed a slab. Three changes together took it to **32-35%** framed on the fall and **4.8-5.1%** at the default wide camera. The flecks are one; each rides a real particle for as long as that particle stays in its band and takes a newly arrived one when it leaves, which is also how foam behaves. The scene's three point lights are the second — the reconstruction leans every tile along the local slope precisely so a light can shade a passing wave, and Riverfall had shipped with no light to do it. The third is the wave itself: the phase speed went from 5 to 12 and the tilt gain from 8 to 16, while the wavelength stayed at 20 because it cannot go lower. Tiles are two units apart, and the shortened wavelengths measured a p95 adjacent-height delta of 0.1178 at 8 units, 0.0989 at 13, and 0.0874 at 16 against the 0.08 checkerboard gate.

A fleck is a presentation of the fluid, exactly as a tile is: it carries no mass, never collides, and adds nothing to the closed 576-particle count it is drawn from. Both pools are sized from a measured supply floor rather than from taste — the curtain-and-plunge band never held fewer than 79 particles across a minute of solved play and the drawn reach never fewer than 511 — so every fleck is on real water on every frame, and `riverfall-spray.test.ts` re-runs that minute and fails if the floor moves under a pool. Turn **lighting on** for shaded relief, switch between study edges and game look, and orbit through overhead, longitudinal, and reverse views to read the complete source-to-outflow chain without relying on the scene title. The river is simulated further upstream than it is drawn so its head cannot run dry, and you will never see that lead-in: nothing is rendered over it.

The landscape, river underfill, waterfall underfill, pond underfill, outflow underfill, and two fluid-surface tile sizes remain seven independent scene recipes — the landscape opaque, every water placement translucent at one scene-wide opacity, and the two tile recipes declared `top-film` so a scene draws only each tile's up-facing skin over the same-colour underfill; translucent water also never draws study edges, and the scene session marks every translucent water material single-layer, a Studio-internal presentation in which a depth-only prepass lets exactly one water fragment blend per pixel, so overlapping films no longer double-darken and parting tiles no longer open lighter hairlines — which together is why the sheet reads as one liquid at the declared opacity instead of a grid of outlined glass boxes. The optional ripple specimen remains on the asset shelf rather than overlaid in this scene, and the ten trees reuse the ordinary seed-varying tree recipe. A deterministic 2D PBF thin-sheet/surface model advances 576 half-mass particles through the exact twelve-reach sidecar: river, lip, fall, three widening and narrowing pond reaches, outflow, visible submergence, occluded sink, under-foundation return, occluded source rise, and visible emergence. The live lane carries fixed 5 ms substeps through the shared 60 Hz cadence and reconstructs the current field onto 321 Eulerian tiles. Separately, the determinism fixture records 240 genuine particle keyframes at 25 ms with five fixed 5 ms substeps per keyframe. Both use a fail-closed world-space Wendland-C2 kernel: every cell-frame requires at least two visible particles inside radius 10 and only the nearest eight, ordered by distance then particle id, influence its local tracer, speed, and support-occupancy samples. A hashed downstream coordinate adds a coherent 20-unit presentation carrier whose phase advances from a five-unit surface-wave floor plus the local PBF speed term, and one bounded neighbor pass smooths across the river, lip, fall, pond, and outflow seams. The authored grid exactly tiles all five live scene-recipe water footprints, tile centers move only along fixed outward normals, each film also leans with the reconstructed field's local slope — a declared, gain-scaled, hard-capped tilt with no spin about the normal, which is what lets a light shade a passing wave — so every posed footprint stays bank-contained, and the moving tiles and concealed underfill use the exact same blue. For replay evidence only, a 24-frame cubic Hermite presentation bridge joins the final observed field back to frame zero with matching endpoint slope, and an appended frame-zero pose at 6,000 ms makes the 6,025 ms trace wrap pose-exact rather than visibly snapping.

The fixture solver projects gravity along the domain, applies a fixed-order Jacobi density constraint and XSPH neighbor smoothing, explicitly attaches particles at the lip, resolves dissipative fall-to-pond impact, and uses an occluded external pump to complete the sink/return/source loop. Its tests separately remove gravity, hidden-pump forcing, the density solve, and XSPH while preserving the remaining world, so those are causal fixture checks rather than visual labels; canonical generation also rejects out-of-budget particle accounting, compact-support coverage, density, speed, and boundary results. The surface input hash covers the particle input hash, complete presentation configuration, and every canonical topology field, while a paired near-versus-distant perturbation proves that supported local particle motion affects its tile and an out-of-support change does not. Studio's private live surface defensively copies the generated initial solver state, advances every later state in the browser, and sends the two tile groups through ordinary instanced sparse matrix deltas; renderer/core does not own the solver, and there is no Riverfall renderer branch. The generated cyclic replay remains byte-pinned fixture evidence and is never the shelf scene's source of motion. Fixed lit and unlit browser views verify that the reconstructed surface and underfill share one exact blue instead of reading as separate droplets, while a twelve-phase default-camera union requires at least half of the stable water pixels to change by four or more RGB steps in the lit phases — the single-blend film shades each wave once, so the former ten-percent-at-eight-steps bar measured the removed double blend rather than the ripple — and replay tests require every reach to participate, every cell to have substantial full-cycle amplitude, the all-frame adjacent-height p95 to remain bounded, and the cyclic seam to stay below its temporal displacement budget. The carried tracer, support-occupancy proxy, authored carrier wave, neighbor smoothing, and loop-closing bridge are presentation devices driven or modulated by accepted local solver observations; they are not a solved water height, energy or density field. This remains a bounded deterministic 2D surface/thin-sheet visualization, not volumetric or free-surface Navier-Stokes, a continuously deforming mesh, transparency or refraction, erosion or hydrology, or gameplay water state.

#### Machine Works assembly proof

Open **Scenes → Machine works**, turn the **simulation on**, and press **Play**. The machine is solved in your browser as it runs — there is no timeline to scrub, and the committed 30-second trace beside it is determinism evidence rather than this scene's source of motion. A cycle starts with a product base on an axis-constrained dynamic carrier while 58 visible slats, each 26 voxels deep, circulate around two internal drive drums; each underside follows the nominal 2.75-unit pitch-drum datum and the articulated straight and turn gaps are bounded. The exact sidecar OBB/SAT regression checks all 58 slats against both drums at 32 phases, proving boundary-only slat contact at the drum end cheeks, no positive-volume overlap, at least about 0.275 world units of central-barrel radial clearance, and detection of an intentionally overlapping negative control. Four minimal exterior hub-and-radial flags share the solved drum phase but remain collision-excluded replay witnesses rather than gears or evidence of tooth contact or torque transmission. Rapier belt contact and friction transport the axis-constrained carrier. The purpose-built insertion bridge has four feet on four occupied foundation pads, two rectilinear towers, one load beam, and two narrowed cream fixed stators. Each stator remains inside the empty opening of an orange three-sided moving C-yoke for the whole prescribed stroke; exact swept bounds require at least 0.4 world units of running clearance on every transverse face and zero positive-volume stator/bar overlap. Rear pads and straight bridge faces are visual alignment datums only, and the bridge is not a Rapier body, so the scene claims neither captive guidance nor solved load transfer. The cabinet, overhead bus, fixed servo housings, load beam, and stators form an exact face-connected external actuation route that ends at the fixed-stator/moving-yoke coupling. Each moving head instead carries a precharged local buffer whose internal conduit and ram backing terminate at the pickup plate; the fixture simulates no charging, flexible moving feed, electricity, magnetic force, motor torque, feedback dynamics, or energy use. Both heads start preloaded by fixed joints with each component outer face touching its energized magnetic pickup plate; there is no in-trace grab or jaw motion. At each station the consumer validates position, orientation, relative speed, dwell, and two-voxel insertion into empty clearance. The cap key occupies core layers seven and eight with deliberate lateral assembly clearance, then its crown underside reaches the core top plane as the vertical seat. Before either component becomes an explicit software-welded compound, the trace measures the live body against the canonical merged pose, records actual translation and shortest angular correction, rejects correction above 0.025 world units or 0.03 radians, and rejects maximum solver penetration above the hashed 0.001-world-unit budget; retention is not a solved key latch. The three-piece product is assembled at about 11.67 seconds. Belt contact delivers the carrier to output, where full pose and speed validation precede release at about 18.33 seconds. The chassis-backed orange trunnion axle extends beyond both belt edges into two open C-bearing cradles whose plinths terminate on separate occupied foundation guard-top faces. Each bearing remains at least 0.5 world units outside the belt, while a third grounded foot supports the outboard servo and its safety coupler face-contacts the axle. Canonical validation axially separates every non-axle carrier solid from the dock, excludes positive-volume dock overlap with the foundation and bucket, leaves about 0.1528 world units around the axle's complete quarter-turn swept cylinder, and analytically checks the complete carrier sweep rather than sampled angles, retaining about 0.7528 world units from the foundation and 0.6000 from the bucket. Before the position command rotates the carrier, the accepted live release pose repeats the quaternion-aware continuous sweep with at least 0.14 world units of dock clearance, about 0.7518 from the foundation, and about 0.5990 from the bucket; the dock still supplies no Rapier revolute constraint, bearing response, motor torque, feedback, or energy model. The product hits the bucket at about 20.87 seconds and satisfies the declared sensor at about 23.87 seconds with the hashed 0.05-unit containment tolerance. The final frame holds until the discrete reset at exactly 30.0 seconds. Bare `Space` pauses or resumes the live solve; the lighting toggle changes illumination only. A live scene has no timeline, so a reproducible moment is a step count rather than a seek — `window.voxelStudio.livePhysics().stepped` is what the browser tests settle against.
...
The scene draws exactly the frame the fixture simulates and validates. The re-layout that made this possible keeps every still solid out of the moving belt band: the press bridge stands wholly behind the lane with its four feet on rear foundation pads, both foundation end frames stop at an open portal the drum turns rotate through, the slimmed drum cheeks stay z-clear of the side guards, the stator blades hang from the load beam over the head stroke only, and the drawn dock stops 0.2 world units short of the recorded bucket's painted west face. Every model's painted content fills its recipe box with the sidecar body origin at its center, so the drawn instance and the validated colliders are the same geometry — the earlier 0.02 still-settle workaround and its pinned debt lists are gone. `sceneSurfaceFightsV1` samples every replay-driven placement at its recorded poses — tilted poses included, by the exact box test — and reports both co-existence (a recorded body entering a still body's space beyond the numerical contact slop) and visibility fights (a recorded face on a still face's plane, facing the same way); flush resting contact stays allowed, shared space does not. The scene test pins Machine Works at zero on both counts, and the Examine pane announces any shared space or fighting surfaces whenever a scene opens — for this scene it now has nothing to announce.
...
The current renderer is an internal Studio-only WebGL2 forward+ proof. It bins visible lights into 48-by-48-drawing-buffer-pixel tiles and 24 depth slices, stores light records and cluster indices in data textures, and runs a fixed shader loop of at most 32 local light evaluations per fragment, so authoring or moving more lights does not create a shader variant per count. The 4,096 authored-light ceiling is not permission for 4,096 lights to overlap one fragment: preparation rejects a cluster above 32 influences, excessive cluster work, excess unbounded-range lights, or an oversized viewport with a specific error and retains the previously prepared lighting instead of silently dropping an influence. Dense finite-light scenes open from a conservative origin-centred view that is swept by the built-in safety proof, but that envelope is only an initial camera: right-drag, WASD, orbit, pitch, and zoom then use the ordinary 0.25-to-256 orbit contract and present the exact requested camera. A successful clustered-light preparation keeps that exact camera, including safe panned centres and views beyond the former 80-unit cap; a rejected preparation restores the prior camera and raster with a specific error. Lighting and projection toggles likewise preserve the current camera and roll back the toggle if its exact frame is unsafe instead of silently moving the view. A scene with unbounded sources or a larger or more overlapping authored field remains governed by the same explicit cluster and work limits. All light handles share one `InstancedMesh`, so they add one marker draw call rather than one object and draw call per light; disabling lighting changes that shared batch to a subdued edit-handle presentation without allocating another material or program.
...
into a single image, and `game` drives the Harbor fixture. Rebuilding
recipe-backed models and comparing them against their saved grids is its own
driver, `npm run studio:recipes` (`scripts/studio-recipes.mjs`). A game's own
