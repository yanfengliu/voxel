...
The scene draws the still foundation and press bridge settled 0.02 world units below the recorded process (`MACHINE_WORKS_STILL_SETTLE_V1`), because the recorded slat tops and the drawn bridge-pad tops used to share the exact y = 9 plane and flickered against each other at both tower feet. The consumer fixture keeps simulating and validating at the process datums (`MACHINE_WORKS_PROCESS_LAYOUT_V1`), so the committed trace and its hashes are untouched by the presentation settle; the output dock also keeps the process height because its bearing bores hold the recorded carrier's trunnion points. `sceneSurfaceFightsV1` samples every replay-driven placement at its recorded poses — tilted poses included, by the exact box test — and reports both co-existence (a recorded body entering a still body's space beyond the numerical contact slop) and visibility fights (a recorded face on a still face's plane, facing the same way) — flush resting contact stays allowed, shared space does not. The machine was authored with its recorded process threading through the still scenery, and the strict rule makes that visible: the belt band runs through the foundation's trough and pads, both drums sit deep inside the foundation's end housings, the settled press feet dip 0.02 into the recorded belt band, the carriage transits through the press towers' front legs and into the dock's reach, each insertion head enters the press volume on its stroke, and the dock's east cells sit under the recorded bucket's rim flare. These known debts are pinned as a shrink-only list in the scene test (anything new fails immediately), the Examine pane announces them whenever the scene opens, and the machine re-layout task owns driving the list to zero by moving statics and re-recording the trace.
