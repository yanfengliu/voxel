...
The separate **animation enabled/disabled** button is a persisted scene-wide movement preference shared by every scene. On a motion-bearing scene, ordinary Play enables the preference and resumes moving point-light sources, animated model placements, and consumer pose replays from the presented phase, while Pause disables it and holds all three; the button reports that saved permission, and Play/Pause reports whether the transport is advancing right now. Exact-time harness draws, timeline seeks, and rejected frames pause the transport transiently at the last phase they actually presented without rewriting an enabled preference, so Play remains the explicit way to resume that inspection pause. A newly opened or reloaded animated scene honors the stored choice; a fresh Studio defaults animation enabled, while a legacy stored view without the field migrates its former moving-light behavior from the stored lighting choice before subsequent writes become independent. When browser storage is unavailable, the Studio remains usable but falls back to the defaults on the next visit.

#### Riverfall fluid replay proof

Open **Scenes → Riverfall canyon**, turn **animation enabled** on, and press **Play**. The default front-left camera shows a high river between asymmetric seeded tree rows, a blue curtain dropping between cliff shoulders, a lower pond with four foam cues, a narrow outflow through the front bank, and the visible subset of 96 pale-cyan particle witnesses. Turn **lighting on** for shaded cliff and tree faces, switch between study edges and game look, scrub the six-second window to inspect evolving solver states, and orbit through overhead, longitudinal, and reverse views to read the complete source-to-outflow chain and the discrete reset without relying on the scene title.

The landscape, river, waterfall, pond, outflow, particle witness, and foam remain seven independent opaque shelf recipes, and the ten trees reuse the ordinary seed-varying tree recipe. Their voxel geometry stays static. A fixture-local deterministic 2D PBF thin-sheet/surface model instead advances 288 fixed-mass particles through the exact twelve-reach sidecar: river, lip, fall, three widening and narrowing pond reaches, outflow, visible submergence, occluded sink, under-foundation return, occluded source rise, and visible emergence. It records 600 genuine frames at 10 ms with two fixed 5 ms substeps per frame, then selects 96 stable particle witnesses for the generated hash-bearing replay. Frame 599 is not replaced with frame zero; Studio holds that genuine final state for the last 10 ms and resets discretely at six seconds. The same scene remains structurally legible with animation disabled.

The fixture solver projects gravity along the domain, applies a fixed-order Jacobi density constraint and XSPH neighbor smoothing, explicitly attaches particles at the lip, resolves dissipative fall-to-pond impact, and uses an occluded external pump to complete the sink/return/source loop. Its tests separately remove gravity, hidden-pump forcing, the density solve, and XSPH while preserving the remaining world, so those are causal fixture checks rather than visual labels. Studio's private `studio.scene/4` defensively owns the generated replay and sends its witness poses through ordinary sparse instance deltas; Voxel does not run the solver, and there is no Riverfall renderer branch. This is a bounded deterministic 2D surface/thin-sheet demonstration over static opaque recipes, not volumetric or free-surface Navier-Stokes, deforming water geometry, transparency or refraction, erosion or hydrology, or gameplay water state.

#### Machine Works assembly proof

Open **Scenes → Machine works**, turn **animation enabled** on, and press **Play**. The 30-second replay starts with a product base on an axis-constrained dynamic carrier while 58 visible slats, each 26 voxels deep, circulate around two internal drive drums; each underside follows the nominal 2.75-unit pitch-drum datum and the articulated straight and turn gaps are bounded. The exact sidecar OBB/SAT regression checks all 58 slats against both drums at 32 phases, proving that slat edges only share a boundary with the drum end cheeks, no positive-volume overlap exists, the central barrel keeps at least about 0.275 world units of radial clearance, and an intentionally shifted negative control is caught. Four exposed axle cogs outside the underframe share the same phase, making 64 synchronized presented elements, but their four non-interacting replay tracks derive from the solved drive-drum poses and their exact cataloged sidecar is never ingested into Rapier, so they demonstrate visible axle phase rather than tooth contact or torque transmission. Only Rapier contact and friction on the driven belt path transport the carrier, whose locked axes align with the visible foundation guards without claiming physical guide contact. The first insertion head lowers the core, the second lowers the cap, and the named mating checks complete the three-piece signal module at about 11.67 seconds. Belt contact then delivers the carrier to the output handoff; only after the fixture validates X, Y, Z, orientation, and speed does the explicit tip servo capture that exact accepted physical pose, begin without a snap, and release the carriage-to-product joint at about 18.33 seconds. The still-colliding carrier tips around its visible hinge, the product hits the bucket at the solver-recorded 20.95-second manifold, and the declared-sensor inside-and-slow check emits collection at about 24.15 seconds with its hashed 0.05-unit containment tolerance. The final recorded frame remains held through the end of the window; reaching exactly 30.0 seconds performs one discrete reset to the first frame instead of interpolating the product backward across the factory. Pause or scrub the shared timeline to inspect any exact phase; the lighting toggle changes only illumination.
...
The **Machine Works** model shelf contains ten independent rebuildable recipes: conveyor underframe, belt slat, internal drive drum, exposed axle phase cog, collection bucket, transfer carrier, insertion head, product base, product core, and product cap. Open one there and enable **colliders** to inspect the sidecar's compound solid or sensor shapes and attachment ports; the 26-voxel-deep slat exposes its named `belt-contact-top` and `drum-pitch-underside` ports, the drive drum exposes `belt-pitch-top`, the carrier its contact underside, and the bucket its interior collection sensor. The exposed cog also has exact authored kinematic collider geometry for reuse and overlay inspection, but this fixture never ingests that sidecar into Rapier because its four scene occurrences are phase-derived replay witnesses rather than a contact mechanism. The source sidecars additionally declare body type, density, friction, restitution, damping, and the product base's CCD request for the fixture adapter, but the current overlay does not display every numeric property. These sidecars remain authoring data and do not run a solver.

The replay is generated headlessly by `fixtures/machine-works-consumer/machine-works-simulation.ts` using pinned `@dimforge/rapier3d-compat` `0.19.3`, then committed as hash-bearing Float32 observations for 71 placement tracks across 1,800 fixed-step frames; Studio adds the two static supports for 73 rendered instances. Its input hash covers the ten sidecars and grains, the stable nine-sidecar Rapier creation order, the exposed cog's explicit phase-derived non-interacting replay-witness mapping, the closed belt path, 26-voxel slat depth, slat pitch and bounded articulation clearances, the nominal 2.75-unit drive-drum pitch datum, the zero-positive-volume-overlap sidecar compounds, the shared 64-element presented slat-drive-drum-exposed-cog phase, belt controller, axis constraints aligned with the visible guards, station targets and full X/Y/Z/orientation/speed handoff tolerances, insertion trajectories, joint ports, contact flags, material values, mating tolerances, merge rule, no-snap output servo, CCD request, both causal-ablation configurations and exact body/joint mappings, and collection thresholds; code generation derives solver, gravity, law, capability, and hash claims from the trace. The exact OBB/SAT regression separately samples every slat, both pitch drums, and 32 phases, requires boundary-only slat-edge/end-cheek contact, records at least about 0.275 world units of central-barrel radial clearance, and includes an overlapping negative control. The final hash also covers belt-contact samples, drive speed and travel, all four phase-derived cog tracks, and both complete-causal-world 240-tick ablations with their maximum-displacement observations: zero drive must remain within 0.05 world units, and driven zero friction must remain within 20 percent of the driven trace over the same interval. The reusable downstream pattern is ordinary rebuildable recipes plus exact physical sidecars plus a consumer-owned deterministic V4 trace presented through sparse instance deltas; no renderer-specific Machine Works mechanism decides the outcome. The proof covers fixed-step rigid-body gravity, exact compound primitive contact, frictional belt transport of an axis-constrained dynamic carrier, restitution, continuous collision detection on the released product, kinematic belt and insertion actuation, exact shared axle-phase animation for the non-interacting exposed-cog replay witnesses, validated fixed-joint-to-compound assembly, the post-arrival no-snap output servo, and declared-sensor bucket containment with the hashed 0.05-unit solver tolerance. It does not prove physical side-guide contact, exposed-cog contact, cog torque transmission, belt tension or compliance, tooth engagement, arbitrary-load no-slip behavior, arbitrary machinery, deformable bodies, fluids, thermodynamics, energy conservation across every solver approximation, or “all physics laws,” and Voxel never derives a physical outcome from what it renders.
...
The harness drives every one of these, so an agent can arrange and manage an ordinary scene the way a person does and read the result back: `scenes()` lists the mount's working collection; `openScene(id)`, `renameScene(id, label)`, and `deleteScene(id)` manage it; `sceneState()` is the open scene as plain data; `selectPlacement(id)` / `selectedPlacement()` are the shared selection; `editScene(next)` commits an add/move/turn/remove (recording one undo step); `undoScene()` / `redoScene()` step the history; `setSnapToGrid(on)` / `snapToGrid()` are the snap flag; `viewState()` and `viewCenter()` separately report the orbit and translated world center; `setViewCenter([x, currentY, z])` drives ground-plane translation through the same camera/light safety path as right-drag and WASD and rejects a vertical jump; and `drawAt(timeMs)` deterministically draws moving lights and returns the clustered-light work metrics for that frame. While a V4 consumer replay is open, `drawAt(timeMs)`, Play, pause, timeline seeks, view controls, inspection reads, and `deleteScene(id)` remain valid, while selection, snap, rename, and edit calls reject rather than mutating or outlining stale authored transforms.
