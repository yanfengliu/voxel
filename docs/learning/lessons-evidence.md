...
The general form: a displacement channel needs something to reveal it. One flat colour with no lighting variation leaves exactly one revealer, the outline, and interior motion is free of visual consequence however large it gets.

The scene's own `surfaceTilt` doc comment says the lean exists so "a light can shade a passing wave" — and Riverfall declared no lights at all, so the channel resolved to nothing however hard it was driven. Turning the studio's lighting on took the same wave field and the same solver from 0.6–1.8% to **7–11%**, and rendered the canyon nearly black, because with no light in the scene ambient was all there was. Three point lights and the flecks together reached **32–35%** framed on the fall. A feature written for a light needs the light to exist; nothing in the code says one is missing, because the arithmetic runs perfectly and multiplies by zero at the end.

Anchor: the measurements above, taken 2026-08-14 against `riverfall-fluid-config.ts` at commit `e6769b5`, and the lit figures against `riverfall-scene.ts`'s `RIVERFALL_LIGHTS_V1` the same day.
