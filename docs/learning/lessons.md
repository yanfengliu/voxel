...
- A multiple of the work is not a floor: contention costs a fixed delay on top of the multiplier, so short tests stretch furthest and a budget must be an allowance *plus* a multiple. ([evidence](lessons-evidence.md#a-budget-below-the-shared-floor-is-worse-than-none-and-a-multiple-of-the-work-is-not-a-floor))
- A budget below the shared default is worse than none — it opts its test out of the floor every other test gets, and surviving the load you happened to test at is the same bomb with a longer fuse. ([evidence](lessons-evidence.md#a-budget-below-the-shared-floor-is-worse-than-none-and-a-multiple-of-the-work-is-not-a-floor))
