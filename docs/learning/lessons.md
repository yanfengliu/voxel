...
- A fix landed in one of two parallel lanes is half a fix: the other lane keeps the defect until someone reads both, so grep for the copy the day of the fix — or make the mechanism exist once. ([evidence](lessons-evidence.md#a-fix-landed-in-one-of-two-parallel-lanes-is-half-a-fix))
...
- Moving geometry that is all one flat colour is visible only at its silhouette; without contrast or a light, more displacement buys nothing. ([evidence](lessons-evidence.md#riverfalls-water-moves-and-only-its-silhouette-shows-it))
- A display channel built for a light needs a light in the scene, or it resolves to nothing however hard it is driven. ([evidence](lessons-evidence.md#riverfalls-water-moves-and-only-its-silhouette-shows-it))
- Sampling limits are the ceiling on how fine a moving pattern can be: past a few samples per cycle, neighbours disagree and the field reads as noise. ([evidence](lessons-evidence.md#the-tile-pitch-is-the-ceiling-on-the-wavelength-and-it-had-none-to-spare))
- A control wired to one subsystem answers for one subsystem, and reads as an answer for the effect. Two producers of one visible behaviour need one switch, or the scene with no authored motion gets no switch at all. ([evidence](lessons-evidence.md#two-switches-for-one-question-left-the-most-simulated-scene-with-none))
- A comparison between a moving thing and a still one measures the movement; when several cases return the same number, that number is what they share, not what distinguishes them. ([evidence](lessons-evidence.md#a-visual-gate-measured-the-sails-and-four-variants-agreed-to-four-decimals))
- A ratio needs both terms from the same source: a log gives a duration and never the budget it was measured against, so "at 100% of its budget" was an assumption wearing a measurement's clothes. ([evidence](lessons-evidence.md#a-log-gives-a-duration-never-the-budget-it-was-measured-against))
