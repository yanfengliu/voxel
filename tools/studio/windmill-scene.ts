...
export const WINDMILL_SCENE_SUMMARY =
  ...
  + 'Five wheat sacks queue at the visible infeed, one slides to the anvil for a blow and is set aside spent, and the flour level in the outfeed bin rises one step for each sack milled; the mill strikes faster than a sack can be dragged clear, so it answers roughly every second blow and stops once the magazine is empty. That material flow is authored presentation kinematics keyed to the blows the hammer actually lands, so it waits until the mill has struck twice and a beat can be measured, and it stops advancing when the mill stops. It is not simulated milling, and the solve proves wind, rotor, cam, hammer, and anvil dynamics and nothing about grain or flour. '
