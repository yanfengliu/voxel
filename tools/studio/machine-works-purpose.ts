...
export const MACHINE_WORKS_PURPOSE_MAP_V1: readonly MachineWorksPurposeEntryV1[] =
  Object.freeze([
    ...
    entry({
      ...
      purpose: 'Presents a bounded kinematic actuation study for both insertion tools through four foundation feet, two towers, one load beam, and fixed stator blades hung inside disjoint moving C-yokes; its cabinet and face-connected bus identify the external service source.',
      locationDatum: {
        frame: 'scene-world',
        anchor: 'four named feet on occupied foundation pads behind the belt band, with stator blades and alignment faces centered at coreStationX and capStationX behind the carrier centerline',
      ...
      mechanicalRelationships: [
        ...
        },
        {
          ...
          evidence: 'The load beam hangs two fixed stator blades through empty three-sided yoke cavities over the head stroke; exact swept bounds prove the occupied stator and yoke bars never overlap, but no captive constraint is claimed.',
        },
        {
          verb: 'aligns',
    ...
    entry({
      ...
      purpose: 'Carries one preloaded component on a contacting energized magnetic pickup face supplied by a precharged head-local buffer, uses orange moving C-yoke faces to expose the pinch boundary around the cream fixed stator blade, follows a prescribed vertical stroke to the keyed insertion datum, and retracts after release.',
      locationDatum: {
        frame: 'named-ports',
        anchor: 'coreStationX or capStationX with the pickup face aligned to the component datum, actuator yoke around the matching stator blade, and rear pads on the matching alignment faces',
...
export const MACHINE_WORKS_FEATURE_PURPOSES_V1: readonly MachineWorksFeaturePurposeV1[] =
  Object.freeze([
    featurePurpose({ id: 'machine-works:feature-purpose:drive-radial-phase-flag',
      recipeId: 'studio:machine-works:drive-cog',
      steps: [boxPurposeStep([0, 0, 0], [3, 3, 3], 'safety')],
      ...
      recipeId: MACHINE_WORKS_PRESS_BRIDGE_RECIPE_ID,
      steps: [
        boxPurposeStep([5, 7, 0], [1, 8, 1], 'detail'),
        boxPurposeStep([19, 7, 0], [1, 8, 1], 'detail'),
      ],
      ...
      recipeId: 'studio:machine-works:insertion-head',
      steps: [
        boxPurposeStep([3, 4, 0], [5, 3, 2], 'detail'),
        boxPurposeStep([5, 4, 2], [1, 14, 1], 'detail'),
      ],
      ...
      recipeId: MACHINE_WORKS_OUTPUT_DOCK_RECIPE_ID,
      steps: [boxPurposeStep([4, 2, 24], [1, 2, 1], 'safety')],
      ...
      recipeId: MACHINE_WORKS_OUTPUT_DOCK_RECIPE_ID,
      steps: [
        boxPurposeStep([0, 0, 25], [2, 1, 2], 'detail'),
        boxPurposeStep([0, 1, 25], [1, 3, 2], 'detail'),
        boxPurposeStep([0, 4, 25], [2, 1, 2], 'detail'),
      ],
...
export const MACHINE_WORKS_PURPOSE_BOUNDARIES_V1 = Object.freeze({
  ...
  pressBridge: 'The exact press-bridge sidecar is hashed and its feet, face-connected external service route, alignment faces, hanging stator blades, empty C-yoke cavities, and disjoint swept yoke bars are tested. The bridge is not ingested into Rapier, so it remains a kinematic actuation and intended-reaction-path study without proving a captive constraint, solver load transfer, or stress.',
