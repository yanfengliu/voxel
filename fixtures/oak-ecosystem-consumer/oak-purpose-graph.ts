...
const BOUNDARY = Object.freeze({
  ...
  postPrimaryCarbonSink: nodeId('sink', 'post-primary-carbon-overflow'),
...
export function createOakEcosystemPurposeGraphV1(): PurposeGraphV1 {
  return purposeGraphV1(SYSTEM, [
    ...
    purposeNodeV1({
      id: nodeId('solid', 'cylindrical-wood-and-root'),
      kind: 'solid',
      label: 'Piecewise-cylindrical woody axes',
      job: 'Rasterize each constant-section stem, branch and root unit into one connected exact-cube material lattice.',
      requiredBy: [NEED.legibility],
      evidence: bound(
        'oak-tissue-material-law.test.ts/proves exact public cubes, one structural material union, retained sources and declared fused ports',
        'Every source survives one connected shared-lattice union with declared parent and child port claims.',
      ),
      honestyBoundary: 'Piecewise-cylindrical quantized cube masks without node flare, not scanned bark, vascular anatomy, cambial growth or mechanics cells.',
    }),
    purposeNodeV1({
      id: nodeId('solid', 'lobed-leaves'),
      kind: 'solid',
      label: 'Lobed, cambered leaves with visible axes',
      job: 'Carry three oak-specific 9/11-lobe variants, petioles, midribs, sparse secondary-vein colour rhythm and material state in one cube batch.',
      requiredBy: [NEED.legibility],
      evidence: bound(
      ...
      honestyBoundary: 'Deterministic voxel variants; secondary-vein colour is not vascular transport, individual venation, damage or scanned anatomy.',
    }),
    purposeNodeV1({
    ...
    purposeBoundaryV1({
      id: BOUNDARY.postPrimaryCarbonSink,
      kind: 'material-sink',
      label: 'Post-primary carbon boundary export',
      job: 'Keep the fixed first-year topology bounded after every authored primary sink completes.',
      quantity: 'carbon',
      visibility: 'invisible',
      truncates: 'Secondary cambial growth, storage organs and carbon export beyond the first-year slice.',
      requiredBy: [NEED.causality, NEED.boundary],
      evidence: bound(
        'oak-post-primary-carbon.test.ts/freezes primary structure and names bounded post-primary carbon export',
        'A no-overflow counter-run stockpiles carbon while the baseline exports only excess above its declared mobile reserve.',
      ),
      honestyBoundary: 'An explicit truncation boundary, not respiration or a secondary-growth model.',
    }),
    purposeBoundaryV1({
  ...
  ], [
    {
      ...
      sinkIds: [BOUNDARY.carbonSink, BOUNDARY.postPrimaryCarbonSink],
      statement: 'Initial carbon plus photosynthesis minus respiration and explicit post-primary export equals current storage.',
    },
    {
      quantity: 'water',
      closed: false,
