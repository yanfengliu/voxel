...
export async function mountWindmillSceneReviewVariant(
  ...
  return page.evaluate(async ({ requestedVariantId }) => {
    ...
    try {
      ...
      // Held still, and its counterpart mount is held the same way. Both sides
      // used to keep turning at unrelated phases, and the difference that
      // measured was mostly where the sails had reached.
